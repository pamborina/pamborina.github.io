...
import { MapPin, Search, Bell, ChevronDown, Sparkles, ShoppingBag, Home, UtensilsCrossed, Store, Package } from 'lucide-react';
...
export const StickyHeader: React.FC<StickyHeaderProps> = ({
  ...
  return (
    <header
      ref={headerRef}
      className={`sticky top-0 z-40 w-full bg-[#140E0A]/95 backdrop-blur-xl border-b border-[#D4AF37]/25 shadow-xl dir-rtl transition-all duration-300 ease-in-out transform ${
        isVisible
          ? 'translate-y-0 opacity-100 pointer-events-auto'
          : '-translate-y-full opacity-0 pointer-events-none'
      } ${isScrolled ? 'shadow-2xl border-[#D4AF37]/40 bg-[#120B07]/98' : ''}`}
    >
      <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2.5 space-y-2">
          ...
          >
            ...
            {onOpenOrderTracking && (
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 rounded-xl bg-[#221710] border border-[#D4AF37]/40 hover:border-[#D4AF37] text-[#FFF1C5] hover:text-[#F4E08B] hover:bg-[#2B1B10] transition-all active:scale-95 cursor-pointer shadow-sm text-xs font-bold"
                title="تتبع حالة طلبك بالرقم"
              >
        ...
        {onTabChange && (
          <div className="hidden md:flex lg:hidden items-center justify-center pt-1">
            <nav className="flex items-center gap-2 bg-[#1A120B] p-1.5 rounded-2xl border border-[#3D2C1E] shadow-md w-full max-w-lg justify-center">
              ...
              <button
                onClick={() => onTabChange('branches')}
                className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  activeNavTab === 'branches'
                    ? 'bg-gradient-to-r from-[#D4AF37] to-[#F4E08B] text-black shadow-md font-extrabold'
                    : 'text-[#C8BFB0] hover:text-[#FFF1C5] hover:bg-[#281C13]'
                }`}
        ...
        {/* 📱 زر تحميل تطبيق بامبورينا الفاخر */}
        <InstallAppButton />
      </div>
    </header>
  );
