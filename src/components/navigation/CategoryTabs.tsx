...
export const CategoryTabs: React.FC<CategoryTabsProps> = ({
  ...
  const { isVisible } = useScrollHeader(15, 6);
  ...
  return (
    <div
      style={{ top: 'var(--sticky-header-height, 140px)' }}
      className={`w-full bg-[#140E0A]/95 backdrop-blur-md border-b border-[#2C1F16] py-2.5 sticky z-30 dir-rtl shadow-md transition-all duration-300 ease-in-out transform ${
        isVisible
              ...
              <span className="relative z-10 flex items-center gap-1.5">
                {category.imageUrl && category.imageUrl.trim() !== '' ? (
                  <img
                    ...
                    className="w-4 h-4 rounded-md object-cover shrink-0 border border-[#D4AF37]/40 shadow-xs"
                    onError={(e) => {
