...
export const CartSheet: React.FC<CartSheetProps> = ({
  ...
  // Intercept phone / browser back button to close Cart sheet without exiting the site
  useModalBackHandler(isOpen, onClose, 'cart');
  ...
  // Delivery Fee is calculated based on distance & kilometers by management
  ...
  // Total Savings
  const totalSavings = couponDiscountAmount + (isFreeDeliveryQualified && standardDeliveryFee > 0 ? standardDeliveryFee : 0);
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-stretch justify-end bg-black/80 backdrop-blur-md dir-rtl overflow-hidden">
          ...
          >
            ...
            <div className="shrink-0 p-4 sm:p-5 border-b border-[#2C1F16] bg-[#120B07] flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#A89C8C] mt-0.5">فرع بامبورينا - الطالبية هرم</p>
                </div>
              </div>
            ...
            {/* 2. Delivery Fee Distance Information Banner */}
            {cartItems.length > 0 && (
              <div className="shrink-0 bg-gradient-to-r from-[#1F150D] to-[#28180E] border-b border-[#2C1F16] p-3 sm:p-3.5 space-y-1">
                <div className="flex items-center gap-2 text-xs font-bold text-[#F4E08B]">
                  <Truck className="w-4 h-4 text-amber-400 shrink-0" />
                  <span>تحديد سعر رسوم التوصيل بناءً على المسافة:</span>
                </div>
                <p className="text-[11px] text-[#C8BFB0] leading-snug">
                  يتم تحديد سعر رسوم التوصيل بناءً على المسافة وعدد الكيلومترات وسيتم تحديدها فوراً من خلال الإدارة وإضافتها للفاتورة.
                </p>
              </div>
            )}
                  ...
                  <div className="p-4 rounded-2xl bg-[#1D140D] border border-[#2D2017] space-y-2.5 text-xs">
                    ...
                    <div className="flex items-center justify-between text-[#C8BFB0]">
                      ...
                      <span className="font-bold text-amber-300 text-xs">
                        تحدد بناءً على المسافة والكيلومترات
                      </span>
                    </div>
                    ...
                    <div className="pt-2 border-t border-[#2C1F16] flex items-center justify-between text-base font-black text-[#FFF1C5]">
                      <div>
                        ...
                        <span className="block text-[10px] text-amber-400 font-normal">
                          + رسوم التوصيل (تحدد فورياً حسب المسافة)
                        </span>
                      </div>
            ...
            {cartItems.length > 0 && (
              <div className="shrink-0 p-4 sm:p-5 border-t border-[#2C1F16] bg-[#120B07] space-y-3 z-20">
                ...
                <div className="p-3 rounded-xl bg-[#140E0A] border border-[#2D2017] text-[#A89C8C] text-[11px] leading-relaxed">
                  <span className="text-[#D4AF37] font-bold">ملاحظة التوصيل:</span> يتم تحديد سعر رسوم التوصيل بناءً على المسافة وعدد الكيلومترات وسيتم تحديدها فوراً من خلال الإدارة وإضافتها للفاتورة.
                </div>
                ...
                <motion.button
                  ...
                  className="w-full min-h-[48px] py-3.5 sm:py-4 px-4 rounded-2xl bg-gradient-to-r from-[#D4AF37] via-[#F4E08B] to-[#D4AF37] disabled:opacity-50 disabled:cursor-not-allowed text-neutral-950 font-black text-sm sm:text-base shadow-[0_4px_35px_rgba(212,175,55,0.4)] hover:brightness-110 flex items-center justify-center gap-2.5 cursor-pointer whitespace-nowrap select-none"
                >
                  <ShoppingBag className="w-5 h-5 text-neutral-950 stroke-[2.5] shrink-0" />
                  <span>
                    {!isStoreOpen ? 'المتجر مغلق مؤقتاً' : isBelowMinOrder ? 'أقل من الحد الأدنى للطلب' : 'متابعة وإتمام الطلب'}
                  </span>
                </motion.button>
              </div>
            )}
