...
export const ImageMagnifier: React.FC<ImageMagnifierProps> = ({
  ...
  if (!src || typeof src !== 'string' || src.trim() === '') {
    return (
      <div className={`w-full p-8 rounded-2xl bg-[#0A0604] border border-[#3D2C1E] text-center text-[#A89C8C] ${containerClassName}`}>
        <p className="text-xs">لا توجد صورة محددة للعرض</p>
      </div>
    );
  }
