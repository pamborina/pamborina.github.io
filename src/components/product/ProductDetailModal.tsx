...
const getGalleryForProduct = (product: Product): string[] => {
  const mainImg = (product.imageUrl && product.imageUrl.trim() !== '')
    ? product.imageUrl
    : ((product.image && product.image.trim() !== '') ? product.image : Images.defaultFood);

  const validGallery = (product.galleryImages || []).filter(
    (url): url is string => typeof url === 'string' && url.trim() !== ''
  );

  if (validGallery.length > 0) {
    return [mainImg, ...validGallery.filter((url) => url !== mainImg)];
  }
  return [mainImg];
};
...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  // Intercept phone / browser back button to close Product detail modal without exiting the site
  useModalBackHandler(isOpen, onClose, 'product');
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-md dir-rtl overflow-hidden">
          ...
          >
                  ...
                  onClick={async () => {
                    if (navigator.share) {
                      try {
                        ...
                        // If user cancels or share is not allowed, fallback to clipboard
                      }
                    }
                    await copyToClipboard(window.location.href);
                    showToast('تم نسخ الرابط بنجاح!', 'يمكنك مشاركته مع أصدقائك الآن', 'success');
                  }}
            ...
            <div className="shrink-0 p-3.5 sm:p-5 border-t border-[#2C1F16] bg-[#120B07] flex items-center justify-between gap-3 sm:gap-4 z-20">
              {/* Quantity Stepper */}
              <div className={`flex items-center gap-1.5 sm:gap-2 bg-[#1A1009] border border-[#3D2C1E] p-1 sm:p-1.5 rounded-2xl shrink-0 ${!isEffectiveAvailable ? 'opacity-40 pointer-events-none' : ''}`}>
                <button
                  type="button"
                  disabled={!isEffectiveAvailable}
                  onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                  className="w-8 h-8 rounded-xl bg-[#221710] flex items-center justify-center text-white hover:text-[#D4AF37] transition-all active:scale-90 disabled:cursor-not-allowed cursor-pointer"
                >
                  <Minus className="w-4 h-4" />
                </button>
                <span className="w-7 sm:w-8 text-center text-xs sm:text-sm font-black text-[#FFF1C5]">
                  {quantity}
                </span>
                <button
                  type="button"
              ...
              <div className="flex-1 flex items-center justify-end gap-2.5 sm:gap-4 min-w-0">
                <div className="text-left dir-ltr shrink-0">
                  <span className="text-[10px] text-[#8E8373] block leading-none">الإجمالي</span>
                  <span className="text-base sm:text-2xl font-black text-[#F4E08B] font-heading leading-tight">
                ...
                {!isStoreOpen ? (
                  <button
                    ...
                    className="min-h-[44px] px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-2xl bg-rose-950/90 text-rose-300 border border-rose-500/60 font-black text-xs sm:text-base flex items-center justify-center gap-2 cursor-pointer shadow-lg hover:bg-rose-900 transition-all whitespace-nowrap select-none"
                  >
                  ...
                  <button
                    type="button"
                    disabled
                    className="min-h-[44px] px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-2xl bg-neutral-800 text-neutral-400 border border-neutral-700 font-bold text-xs sm:text-base flex items-center justify-center gap-2 cursor-not-allowed whitespace-nowrap select-none"
                  >
                  ...
                  <motion.button
                    ...
                    className="min-h-[44px] px-4 sm:px-7 py-2.5 sm:py-3.5 rounded-2xl bg-gradient-to-r from-[#D4AF37] via-[#F4E08B] to-[#D4AF37] text-neutral-950 font-black text-xs sm:text-base shadow-[0_4px_25px_rgba(212,175,55,0.4)] hover:brightness-110 flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap select-none"
                  >
                    <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5 text-neutral-950 stroke-[2.5] shrink-0" />
