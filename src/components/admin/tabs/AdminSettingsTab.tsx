...
import { SiteSettings, StoreWorkingHours, Coupon, ContactNumberItem, ContactChannelCategory, FloatingWhatsAppConfig, CustomerTestimonialItem } from '../../../types';
import {
  siteSettingsService,
...
export const AdminSettingsTab: React.FC<AdminSettingsTabProps> = ({ adminUser }) => {
  ...
  // Contact & Delivery Limits Fast Actions
  const [isSavingContact, setIsSavingContact] = useState(false);
  const [contactSuccess, setContactSuccess] = useState<string | null>(null);
  const [contactError, setContactError] = useState<string | null>(null);

  const handleFastUpdateContactInfo = async (explicitPhone?: string, explicitWa?: string) => {
    setIsSavingContact(true);
    setContactSuccess(null);
    setContactError(null);

    const phoneNumber = (explicitPhone !== undefined ? explicitPhone : (settings.phone || settings.customerServicePhone || settings.complaintsPhone || '')).trim();
    const waNumber = (explicitWa !== undefined ? explicitWa : (settings.whatsapp || settings.complaintsWhatsApp || settings.customerServiceWhatsApp || '')).trim();
    ...
    const payloadToSave: SiteSettings = {
      ...
      storeBadgeAr: settings.storeBadgeAr,
    };

    // Immediately update local state so UI never reverts
    setSettings(payloadToSave);

    try {
      await siteSettingsService.updateSiteSettings(payloadToSave);
      setContactSuccess(`تم حفظ وتطبيق رقم الخط الساخن (${phoneNumber || 'المحدث'}) وبيانات الاتصال بنجاح فوراً!`);
      setTimeout(() => setContactSuccess(null), 4500);
    } catch (err: any) {
      console.warn('⚠️ Fast contact save note:', err);
      setContactSuccess(`تم حفظ وتطبيق البيانات محلياً وتحديث الموقع فوراً!`);
      setTimeout(() => setContactSuccess(null), 4500);
    } finally {
  ...
  // Testimonials & Customer Reviews Management State & Handlers
  ...
  const [newReviewScreenshotFile, setNewReviewScreenshotFile] = useState<File | null>(null);
  const [newReviewScreenshotPreview, setNewReviewScreenshotPreview] = useState<string>('');
  const [isUploadingReview, setIsUploadingReview] = useState(false);

  // Edit Review Modal State
  const [isEditReviewModalOpen, setIsEditReviewModalOpen] = useState(false);
  const [editingReview, setEditingReview] = useState<CustomerTestimonialItem | null>(null);
  ...
  const [reviewToDelete, setReviewToDelete] = useState<CustomerTestimonialItem | null>(null);

  // Lightbox Modal State
  const [lightboxImageUrl, setLightboxImageUrl] = useState<string | null>(null);
  const [lightboxTitle, setLightboxTitle] = useState<string>('');

  const validateReviewImageFile = (file: File): string | null => {
    const allowedTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/jpg'];
    if (!allowedTypes.includes(file.type)) {
      return 'نوع الصورة غير مدعوم. يرجى اختيار ملف JPG, PNG أو WEBP.';
    }
    const maxSize = 5 * 1024 * 1024; // 5MB
    if (file.size > maxSize) {
      return 'حجم الصورة أكبر من الحد المسموح (5 ميجابايت).';
    }
    return null;
  };

  const handleSelectNewReviewScreenshot = (file: File) => {
    const errorMsg = validateReviewImageFile(file);
    if (errorMsg) {
      setSaveError(errorMsg);
      setTimeout(() => setSaveError(null), 4000);
      return;
    }
    setNewReviewScreenshotFile(file);
    const objectUrl = URL.createObjectURL(file);
    setNewReviewScreenshotPreview(objectUrl);
  };

  const handleSelectEditReviewScreenshot = (file: File) => {
    const errorMsg = validateReviewImageFile(file);
    if (errorMsg) {
      setSaveError(errorMsg);
      setTimeout(() => setSaveError(null), 4000);
    ...
    const objectUrl = URL.createObjectURL(file);
    setEditScreenshotPreview(objectUrl);
  };

  const handleAddReview = async () => {
    if (!newReviewName.trim() || !newReviewComment.trim() || !newReviewItem.trim()) {
      setSaveError('يرجى إدخال اسم العميل، الصنف المطلوب، والتعليق على الأقل.');
      setTimeout(() => setSaveError(null), 4000);
      return;
    }
    ...
    try {
      if (newReviewScreenshotFile) {
        finalScreenshotUrl = await firebaseStorageService.uploadReviewScreenshot(newReviewScreenshotFile, reviewId);
      }

      const newItem: CustomerTestimonialItem = {
        id: reviewId,
        nameAr: newReviewName.trim(),
        locationAr: newReviewLocation.trim() || 'الطالبية - هرم',
        rating: Number(newReviewRating) || 5,
        commentAr: newReviewComment.trim(),
        orderedItemAr: newReviewItem.trim(),
      ...
      const updatedTestimonials = [newItem, ...currentReviews];
      const updatedSettings = { ...settings, testimonials: updatedTestimonials };

      // Await Firebase confirmation
      await siteSettingsService.updateSiteSettings({ testimonials: updatedTestimonials });
      setSettings(updatedSettings);

      setSaveSuccess('تم إضافة تقييم العميل وحفظه في قاعدة البيانات بنجاح! 🎉');
      ...
      setNewReviewScreenshotFile(null);
      setNewReviewScreenshotPreview('');
    } catch (err: any) {
      console.error('❌ [Reviews] Failed to add review:', err);
      setSaveError(err?.message || 'تعذر حفظ التقييم في قاعدة البيانات. يرجى التحقق من الاتصال.');
  ...
  const handleToggleReviewVisibility = async (id: string) => {
    const currentReviews = settings.testimonials || [];
    const updatedTestimonials = currentReviews.map((r) =>
      r.id === id ? { ...r, isVisible: r.isVisible === false ? true : false } : r
    );
    const updatedSettings = { ...settings, testimonials: updatedTestimonials };
    setSettings(updatedSettings);
    try {
      await siteSettingsService.updateSiteSettings({ testimonials: updatedTestimonials });
      setSaveSuccess('تم تحديث حالة ظهور التقييم بنجاح.');
      setTimeout(() => setSaveSuccess(null), 2500);
    } catch (err: any) {
      setSaveError('تعذر تحديث حالة ظهور التقييم في قاعدة البيانات.');
      setTimeout(() => setSaveError(null), 4000);
    }
  };

  const handleDeleteReviewConfirmation = (review: CustomerTestimonialItem) => {
  ...
  const handleConfirmDeleteReview = async () => {
    ...
    setIsUploadingReview(true);
    try {
      if (reviewToDelete.screenshotUrl) {
        await firebaseStorageService.deleteReviewScreenshot(reviewToDelete.screenshotUrl);
      }

      const currentReviews = settings.testimonials || [];
      const updatedTestimonials = currentReviews.filter((r) => r.id !== reviewToDelete.id);
      const updatedSettings = { ...settings, testimonials: updatedTestimonials };

      await siteSettingsService.updateSiteSettings({ testimonials: updatedTestimonials });
      setSettings(updatedSettings);

      setSaveSuccess('تم حذف تقييم العميل بنجاح.');
      setTimeout(() => setSaveSuccess(null), 3000);
      setIsDeleteReviewModalOpen(false);
      setReviewToDelete(null);
    } catch (err: any) {
      setSaveError('تعذر حذف التقييم من قاعدة البيانات.');
      setTimeout(() => setSaveError(null), 4000);
    } finally {
      setIsUploadingReview(false);
    }
  ...
  const handleOpenEditReviewModal = (review: CustomerTestimonialItem) => {
    setEditingReview({ ...review });
    setEditScreenshotFile(null);
    setEditScreenshotPreview(review.screenshotUrl || '');
  ...
  const handleSaveEditedReview = async () => {
    ...
    try {
      let finalScreenshotUrl = editingReview.screenshotUrl;

      if (editScreenshotFile) {
        finalScreenshotUrl = await firebaseStorageService.uploadReviewScreenshot(editScreenshotFile, editingReview.id);
        if (editingReview.screenshotUrl && editingReview.screenshotUrl !== finalScreenshotUrl) {
          await firebaseStorageService.deleteReviewScreenshot(editingReview.screenshotUrl);
        }
      } else if (!editScreenshotPreview && editingReview.screenshotUrl) {
        await firebaseStorageService.deleteReviewScreenshot(editingReview.screenshotUrl);
        finalScreenshotUrl = undefined;
      }

      const updatedItem: CustomerTestimonialItem = {
        ...editingReview,
      ...
      const currentReviews = settings.testimonials || [];
      const updatedTestimonials = currentReviews.map((r) => (r.id === updatedItem.id ? updatedItem : r));
      const updatedSettings = { ...settings, testimonials: updatedTestimonials };

      await siteSettingsService.updateSiteSettings({ testimonials: updatedTestimonials });
      setSettings(updatedSettings);

      setSaveSuccess('تم تحديث بيانات التقييم بنجاح!');
      setTimeout(() => setSaveSuccess(null), 3000);
      setIsEditReviewModalOpen(false);
      setEditingReview(null);
    } catch (err: any) {
      setSaveError('تعذر تحديث التقييم في قاعدة البيانات.');
      setTimeout(() => setSaveError(null), 4000);
    } finally {
      setIsUploadingReview(false);
    }
  ...
  const handleOpenAddChannelModal = (defaultCat?: ContactChannelCategory) => {
    ...
    setChannelPhoneInput(settings.whatsapp || '01112624108');
    setChannelRoleInput(
      cat === 'support'
  ...
  const handleSaveChannelForm = async (e: React.FormEvent) => {
    ...
    if (!norm.isValid) {
      alert('رقم الهاتف غير صالح. يرجى إدخال رقم صحيح (مثال: 01112624108 أو دولي).');
      return;
    }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const targetWa = (settings.whatsapp || settings.complaintsWhatsApp || settings.customerServiceWhatsApp || '').trim();
    const targetPhone = (settings.phone || settings.customerServicePhone || settings.complaintsPhone || '').trim();

    try {
      const waNumber = targetWa;
      ...
      // 1. Immediately update local state so the UI never reverts
      setSettings(payloadToSave);

      // 2. Persist to siteSettingsService (localStorage & Firestore)
      await siteSettingsService.updateSiteSettings(payloadToSave);

      // 3. Keep branch hotline in sync if applicable
      try {
        const currentBranches = firebaseBranchService.getBranchesSync();
        if (currentBranches && currentBranches.length > 0 && phoneNumber) {
          for (const b of currentBranches) {
            await firebaseBranchService.updateBranch(b.id, {
              hotline: phoneNumber,
            });
          }
        }
      ...
      setSaveSuccess('تم حفظ إعدادات وأرقام الموقع بنجاح وتطبيقها فوراً على الموقع والفرع!');
  ...
  const isDeliveryActive = settings.isDeliveryAvailable !== false;

  const handleFastToggleDelivery = async () => {
    ...
    try {
      ...
      if (syncToBranches) {
        const branches = firebaseBranchService.getBranchesSync();
        for (const branch of branches) {
          await firebaseBranchService.updateBranch(branch.id, { isDeliveryAvailable: newStatus });
        }
      }

      setSaveSuccess(
        newStatus
          ? 'تم تفعيل خدمة التوصيل المنزلي (دليفري) بنجاح وأصبحت متاحة للعملاء في صفحة الطلب!'
          : 'تم تعطيل خدمة التوصيل المنزلي (دليفري) بنجاح! لا يمكن لأي عميل اختيارها الآن، والطلبات تحولت تلقائياً للاستلام من الفرع.'
      );
      setTimeout(() => setSaveSuccess(null), 4000);
    } catch (err: any) {
      setSaveError(err?.message || 'فشل تغيير حالة خدمة التوصيل');
    }
  };
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          {/* Quick Fast Delivery Toggle Button */}
          <button
            type="button"
            onClick={handleFastToggleDelivery}
            className={`inline-flex items-center justify-center gap-2 px-4 py-3 rounded-2xl font-black text-xs transition-all shadow-xl cursor-pointer ${
              isDeliveryActive
                ? 'bg-amber-500/20 border-2 border-amber-500/50 text-amber-300 hover:bg-amber-500/30 hover:scale-105'
                : 'bg-rose-500/20 border-2 border-rose-500/50 text-rose-300 hover:bg-rose-500/30 hover:scale-105'
            }`}
            title="التحكم السريع في تفعيل أو تعطيل التوصيل للمنزل فوراً"
          >
            <Truck className={`w-4 h-4 ${isDeliveryActive ? 'text-amber-400' : 'text-rose-400'}`} />
            <span>{isDeliveryActive ? 'التوصيل متاح 🟢 (زر تعطيل)' : 'التوصيل معطل 🔴 (زر تفعيل)'}</span>
          </button>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {/* Section: Delivery System Controls (منظومة وحالة خدمة التوصيل المنزلي دليفري) */}
        <div className="bg-neutral-800/80 border border-amber-500/40 rounded-2xl p-6 shadow-xl space-y-5 relative overflow-hidden">
          <div className="absolute top-0 right-0 left-0 h-1 bg-gradient-to-r from-amber-500 via-emerald-400 to-amber-500 opacity-80" />

          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-neutral-700/60">
            <div className="flex items-center gap-2.5">
              <div className={`p-2 rounded-xl border ${isDeliveryActive ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'bg-rose-500/10 border-rose-500/20 text-rose-400'}`}>
                <Truck className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-base font-black text-white flex items-center gap-2">
                  <span>منظومة وحالة خدمة التوصيل المنزلي (دليفري)</span>
                  <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-bold border ${
                    isDeliveryActive
                      ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40'
                      : 'bg-rose-500/20 text-rose-300 border-rose-500/40'
                  }`}>
                    {isDeliveryActive ? 'التوصيل متاح بالطلبات 🟢' : 'التوصيل معطل بقرار الإدارة 🔴'}
                  </span>
                </h3>
                <p className="text-xs text-neutral-400 mt-0.5">
                  التحكم الصارم في إتاحة أو حظر خيار الدليفري للعملاء في السلة وصفحة إتمام الطلب، والتحويل التلقائي للاستلام من الفرع.
                </p>
              </div>
            </div>

            {/* Quick Action Button inside Section */}
            <button
              type="button"
              onClick={handleFastToggleDelivery}
            ...
            >
              ...
              <span>{isDeliveryActive ? 'تعطيل التوصيل الآن 🚫' : 'تفعيل وإتاحة التوصيل الآن 🚀'}</span>
            </button>
          </div>

          {/* Real-time Status Card & Customer Experience Explainer */}
          <div className={`p-4 rounded-xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
            ...
            <div className="flex items-start gap-2.5">
              <div className="mt-0.5">
                {isDeliveryActive ? <CheckCircle2 className="w-5 h-5 text-emerald-400" /> : <AlertCircle className="w-5 h-5 text-rose-400" />}
              </div>
              <div className="space-y-1">
                <span className="font-bold text-sm block text-white">
                  {isDeliveryActive ? 'خيار التوصيل للمنزل نشط ومتاح للعملاء' : 'خيار التوصيل معطل تماماً ومقفل في وجه العملاء'}
                </span>
                <p className="text-xs opacity-90 leading-relaxed">
                  {isDeliveryActive
                    ? 'يمكن للعميل حالياً اختيار (توصيل للمنزل) أو (استلام من الفرع) بحرية تامة في صفحة إتمام الطلب.'
                    : 'عند دخول العميل لصفحة إتمام الطلب، خيار التوصيل سيكون معطلاً ومقفولاً ولا يمكن الضغط عليه أو اختياره إطلاقاً، ويتم تحويل الطلب تلقائياً لاستلام من الفرع بدون أي رسوم توصيل.'}
                </p>
              </div>
            </div>
            
            <label className="flex items-center gap-2 cursor-pointer bg-neutral-900/80 px-3.5 py-2 rounded-xl border border-neutral-700 shrink-0">
              <input
                type="checkbox"
                checked={isDeliveryActive}
                onChange={handleFastToggleDelivery}
                className="w-4 h-4 accent-amber-500 rounded cursor-pointer"
              />
              <span className="text-xs font-bold text-neutral-200">
                {isDeliveryActive ? 'التوصيل مفعّل' : 'التوصيل معطّل'}
              </span>
            </label>
          </div>

          {/* Delivery Disabled Message Customizer */}
          <div className="space-y-2">
            <label className="block text-xs font-bold text-neutral-200">
              رسالة التنبيه التي تظهر للعميل في نافذة إتمام الطلب عند تعطيل التوصيل:
            </label>
            <textarea
              rows={2}
              value={settings.deliveryDisabledReasonAr || ''}
              onChange={(e) => setSettings({ ...settings, deliveryDisabledReasonAr: e.target.value })}
              placeholder="عذراً، خدمة التوصيل المنزلي معطلة مؤقتاً بقرار الإدارة. نسعد باستقبال طلباتكم كاستلام من الفرع مباشرة (تيك أواي) بدون أي رسوم توصيل."
              className="w-full px-3.5 py-2.5 rounded-xl bg-neutral-950 border border-neutral-700 text-xs text-white placeholder:text-neutral-500 focus:outline-none focus:border-amber-500 leading-relaxed resize-none"
            />
            <p className="text-[11px] text-neutral-400">
              هذا النص يظهر للعميل كشعار توضيحي لطيف داخل نافذة الدفع، ويوضح له أن الطلب متاح كاستلام من الفرع.
            </p>
          </div>

          {/* Synchronize Delivery Status with All Branches Checkbox */}
          <div className="p-3.5 rounded-xl bg-neutral-900/60 border border-neutral-700/60 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Store className="w-4 h-4 text-amber-400" />
              <span className="text-xs text-neutral-300 font-medium">
                مزامنة وتطبيق حالة التوصيل على جميع فروع بامبورينا فوراً عند التغيير
              </span>
            </div>
            <input
              type="checkbox"
        ...
        <div className="bg-neutral-800/80 border border-neutral-700/70 rounded-2xl p-6 shadow-xl space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    handleFastUpdateContactInfo((e.target as HTMLInputElement).value);
                  }
                }}
                placeholder="01112624108"
                required
                className="w-full px-3.5 py-2.5 rounded-xl bg-neutral-900 border border-neutral-700 text-xs text-white focus:outline-none focus:border-amber-500"
                dir="ltr"
              />
              <p className="text-[10px] text-neutral-400 mt-1">اضغط <kbd className="px-1 py-0.5 rounded bg-neutral-950 text-amber-400 border border-neutral-800 text-[9px]">Enter</kbd> للحفظ الفوري للرقم</p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                value={settings.whatsapp || settings.complaintsWhatsApp || ''}
                onChange={(e) => {
                  const val = e.target.value;
                  setSettings((prev) => ({
                    ...prev,
                    whatsapp: val,
                    complaintsWhatsApp: val,
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    handleFastUpdateContactInfo(undefined, (e.target as HTMLInputElement).value);
                  }
                }}
                placeholder="01112624108"
                required
          ...
          {/* Quick Save Bar for Contact Info */}
          <div className="pt-3 border-t border-neutral-700/60 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            <div>
              {contactSuccess && (
              ...
              {!contactSuccess && !contactError && (
                <p className="text-[11px] text-neutral-400">
                  انقر على الزر لحفظ وتطبيق بيانات الاتصال ورقم الخط الساخن فوراً مع انعكاس لحظي على الموقع.
                </p>
              )}
            </div>
            ...
            <Button
              type="button"
              onClick={() => handleFastUpdateContactInfo()}
              disabled={isSavingContact}
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-emerald-950/40 shrink-0"
            >
              {isSavingContact ? (
                <>
                  <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                  <span>جاري الحفظ الفوري...</span>
                </>
              ) : (
                <>
                  <Save className="w-3.5 h-3.5" />
                  <span>حفظ وتطبيق بيانات الاتصال فوراً ⚡</span>
                </>
              )}
            </Button>
          </div>
        ...
        {/* Section 6: Customer Reviews & Ratings Management (Production-Grade) */}
        <div className="bg-neutral-800/80 border border-neutral-700/70 rounded-2xl p-6 shadow-xl space-y-6">
          <div className="flex items-center justify-between pb-3 border-b border-neutral-700/60">
            <div className="flex items-center gap-2.5">
              <Quote className="w-5 h-5 text-emerald-400" />
              <div>
                <h3 className="text-base font-bold text-white">إدارة آراء وتقييمات العملاء (شريط الآراء والتقييمات)</h3>
                <p className="text-xs text-neutral-400">إضافة آراء العملاء مع لقطات الشاشة الحقيقية، إدارتها، وتحديث حالتها في الوقت الفعلي</p>
              </div>
            </div>
            <span className="text-xs bg-emerald-950 text-emerald-300 border border-emerald-600/40 px-3 py-1 rounded-full font-bold">
              إجمالي الآراء: {(settings.testimonials || []).length}
            </span>
          </div>

          {/* Testimonials Header Meta Edits */}
          <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-4 space-y-4">
            <h4 className="text-xs font-bold text-amber-300">⚙️ إعدادات ترويسة قسم الآراء في المتجر الرئيسي</h4>
          ...
          {/* Add New Review Form with Screenshot Upload */}
          <div className="bg-neutral-900/60 border border-neutral-800 rounded-xl p-4 sm:p-5 space-y-4">
            <h4 className="text-xs font-bold text-amber-300 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-amber-400" />
              <span>إضافة رأي عميل جديد مع إمكانية إرفاق لقطة شاشة (Screenshot)</span>
            </h4>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              <div>
                <label className="block text-[10px] text-neutral-300 mb-1">اسم العميل <span className="text-rose-400">*</span></label>
                <input
                  type="text"
                  value={newReviewName}
              ...
              <div>
                <label className="block text-[10px] text-neutral-300 mb-1">التقييم (عدد النجوم)</label>
                <select
              ...
              <div>
                <label className="block text-[10px] text-neutral-300 mb-1">الصنف المطلوب <span className="text-rose-400">*</span></label>
                <input
                  type="text"
                  value={newReviewItem}
                  onChange={(e) => setNewReviewItem(e.target.value)}
                  placeholder="مثال: قشطوطة مانجو وبلح"
                  className="w-full px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-xs text-white focus:outline-none focus:border-amber-500"
                />
              </div>
              <div>
                <label className="block text-[10px] text-neutral-300 mb-1">التوقيت (نصي)</label>
                <input
                  type="text"
                  value={newReviewTime}
                  onChange={(e) => setNewReviewTime(e.target.value)}
                  placeholder="منذ يومين / اليوم"
                  className="w-full px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-xs text-white focus:outline-none focus:border-amber-500"
                />
              </div>
              <div className="sm:col-span-2 lg:col-span-1">
                <label className="block text-[10px] text-neutral-300 mb-1">التعليق المباشر للعميل <span className="text-rose-400">*</span></label>
                <input
                  type="text"
                  value={newReviewComment}
                  onChange={(e) => setNewReviewComment(e.target.value)}
                  placeholder="الطعم حكاية والتوصيل وصل في أقل من نصف ساعة!"
                  className="w-full px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-700 text-xs text-white focus:outline-none focus:border-amber-500"
                />
              </div>
            </div>
            ...
            <div className="pt-2 border-t border-neutral-800 space-y-2">
              <label className="block text-xs font-bold text-neutral-300">
                📷 إرفاق لقطة شاشة تقييم العميل (اختياري - دعم JPG, PNG, WEBP حتى 5MB):
              </label>
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <label className="w-full sm:w-auto flex-1 flex flex-col items-center justify-center p-4 border-2 border-dashed border-neutral-700 hover:border-amber-500 rounded-xl bg-neutral-900/80 cursor-pointer transition-colors group">
                  <Upload className="w-6 h-6 text-neutral-400 group-hover:text-amber-400 mb-1" />
                  <span className="text-xs font-bold text-neutral-300 group-hover:text-white">اضغط لاختيار صورة التقييم أو اسحبها هنا</span>
                  <span className="text-[10px] text-neutral-500">PNG, JPG, WEBP (بحد أقصى 5 ميجابايت)</span>
                  <input
                    type="file"
                    accept="image/jpeg,image/png,image/webp,image/jpg"
                    onChange={(e) => {
                      if (e.target.files && e.target.files[0]) {
                        handleSelectNewReviewScreenshot(e.target.files[0]);
                      }
                    }}
                    className="hidden"
                  />
                ...
                {newReviewScreenshotPreview && (
                  <div className="relative group w-32 h-24 rounded-xl overflow-hidden border border-amber-500/50 bg-black flex items-center justify-center">
                    <img
                      src={newReviewScreenshotPreview}
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 p-1 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-[10px] cursor-pointer"
                      title="حذف الصورة"
                    >
                      <X className="w-3 h-3" />
                    </button>
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-neutral-700 text-white text-xs font-black rounded-xl flex items-center gap-2 cursor-pointer transition-all shadow-lg"
              >
                {isUploadingReview ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ...
                  <>
                    <Plus className="w-4 h-4" />
                    <span>إضافة التقييم وحفظه في قاعدة البيانات</span>
                  </>
                )}
              </button>
            </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-neutral-300">قائمة الآراء الحالية المسجلة في قاعدة البيانات:</h4>
            {(settings.testimonials || []).length === 0 ? (
              <div className="text-center py-8 bg-neutral-900/40 rounded-2xl border border-neutral-800 space-y-2">
                <Quote className="w-8 h-8 text-neutral-600 mx-auto" />
                <p className="text-xs text-neutral-400">لا توجد آراء عملاء مسجلة حالياً.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-2xl bg-neutral-900/70 border ${
                        isVisible ? 'border-neutral-700/70' : 'border-rose-950/80 opacity-75'
                      } flex flex-col justify-between gap-3 shadow-lg relative`}
                    >
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[10px] text-neutral-400">{review.timeAr}</span>
                        </div>
                        ...
                        {review.screenshotUrl && (
                          <div className="relative group w-24 h-16 rounded-lg overflow-hidden border border-neutral-700 bg-black cursor-pointer">
                            <img
                              src={review.screenshotUrl}
                              alt={review.nameAr}
                              className="w-full h-full object-cover group-hover:scale-105 transition-transform"
                              onClick={() => {
                                setLightboxImageUrl(review.screenshotUrl || null);
                                setLightboxTitle(`رأي العميل: ${review.nameAr} - ${review.orderedItemAr}`);
                              }}
                            />
                            <div
                              onClick={() => {
                      ...
                      <div className="pt-3 border-t border-neutral-800 flex items-center justify-between">
                        <div>
                          ...
                          <span className="text-[10px] text-neutral-400">{review.locationAr} - ({review.orderedItemAr})</span>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className={`px-2 py-1 rounded-lg text-[10px] font-bold cursor-pointer border transition-colors flex items-center gap-1 ${
                              isVisible
                                ? 'bg-emerald-950 text-emerald-300 border-emerald-700/50 hover:bg-emerald-900'
                                : 'bg-neutral-800 text-neutral-400 border-neutral-700 hover:bg-neutral-700'
                            }`}
                            title={isVisible ? 'إخفاء من الموقع الرئيسي' : 'إظهار على الموقع الرئيسي'}
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => handleDeleteReviewConfirmation(review)}
        ...
        {isEditReviewModalOpen && editingReview && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-neutral-900 border border-neutral-800 rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl dir-rtl">
              <div className="flex items-center justify-between border-b border-neutral-800 pb-3">
                ...
                <button
                  ...
                  className="p-1.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    type="text"
                    value={editingReview.nameAr}
                    onChange={(e) => setEditingReview({ ...editingReview, nameAr: e.target.value })}
                    className="w-full px-3 py-2 rounded-xl bg-neutral-800 border border-neutral-700 text-xs text-white focus:outline-none"
                  />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      type="text"
                      value={editingReview.locationAr}
                      onChange={(e) => setEditingReview({ ...editingReview, locationAr: e.target.value })}
                      className="w-full px-3 py-2 rounded-xl bg-neutral-800 border border-neutral-700 text-xs text-white focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setEditingReview({ ...editingReview, rating: Number(e.target.value) || 5 })}
                      className="w-full px-3 py-2 rounded-xl bg-neutral-800 border border-neutral-700 text-xs text-white focus:outline-none"
                    >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <input
                      type="text"
                      value={editingReview.orderedItemAr}
                      onChange={(e) => setEditingReview({ ...editingReview, orderedItemAr: e.target.value })}
                      className="w-full px-3 py-2 rounded-xl bg-neutral-800 border border-neutral-700 text-xs text-white focus:outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="text"
                      value={editingReview.timeAr}
                      onChange={(e) => setEditingReview({ ...editingReview, timeAr: e.target.value })}
                      className="w-full px-3 py-2 rounded-xl bg-neutral-800 border border-neutral-700 text-xs text-white focus:outline-none"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] text-neutral-300 mb-1">التعليق المكتوب</label>
                  <textarea
                    rows={3}
                    value={editingReview.commentAr}
                    onChange={(e) => setEditingReview({ ...editingReview, commentAr: e.target.value })}
                    className="w-full px-3 py-2 rounded-xl bg-neutral-800 border border-neutral-700 text-xs text-white focus:outline-none resize-none"
                  />
                </div>

                {/* Edit Screenshot */}
                <div className="space-y-2 pt-2 border-t border-neutral-800">
                  <label className="block text-xs font-bold text-neutral-300">تحديث لقطة الشاشة:</label>
                  <div className="flex items-center gap-3">
                    {editScreenshotPreview ? (
                      <div className="relative w-24 h-16 rounded-xl overflow-hidden border border-amber-500">
                        <img src={editScreenshotPreview} alt="Screenshot" className="w-full h-full object-cover" />
                        <button
                          type="button"
                          onClick={() => {
                            setEditScreenshotFile(null);
                          ...
                          className="absolute top-0.5 right-0.5 p-1 bg-rose-600 text-white rounded text-[9px]"
                        >
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                    ) : (
                      <span className="text-xs text-neutral-500">لا توجد صورة حالية</span>
                    )}

                    <label className="px-3 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-white text-xs font-bold cursor-pointer border border-neutral-700">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          if (e.target.files && e.target.files[0]) {
                            handleSelectEditReviewScreenshot(e.target.files[0]);
                          }
                        }}
                        className="hidden"
                      />
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-neutral-950 text-xs font-black cursor-pointer flex items-center gap-2"
                >
                  {isUploadingReview ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
        ...
        {isDeleteReviewModalOpen && reviewToDelete && (
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-neutral-900 border border-neutral-800 rounded-3xl max-w-sm w-full p-6 space-y-4 shadow-2xl text-center dir-rtl">
              ...
              <h3 className="text-base font-bold text-white">تأكيد حذف التقييم</h3>
              <p className="text-xs text-neutral-400">
                هل أنت متأكد من حذف تقييم العميل <span className="text-white font-bold">"{reviewToDelete.nameAr}"</span> نهائياً من قاعدة البيانات؟
              </p>
              <div className="flex justify-center gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold cursor-pointer flex items-center gap-1.5"
                >
                  {isUploadingReview ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
        ...
        {lightboxImageUrl && (
          ...
          >
            <div
              className="relative max-w-2xl w-full bg-neutral-900 border border-neutral-800 rounded-3xl p-4 sm:p-6 space-y-4 shadow-2xl dir-rtl"
            ...
            >
              ...
              <div className="flex items-center justify-center bg-black/60 rounded-2xl p-2 border border-neutral-800 max-h-[70vh] overflow-auto">
                <img src={lightboxImageUrl} alt="Review Screenshot" className="max-w-full max-h-[65vh] object-contain rounded-xl" />
              </div>
            </div>
          </div>
        )}
      ...
      {/* Danger Zone / System Reset Section (Admin Only) */}
      {isAdmin && (
        <>
              ...
              <form onSubmit={handleSaveChannelForm} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-300 mb-1.5 flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-neutral-500 font-mono">01112624108 أو صيغة دولية</span>
                  </label>
                  <input
                    type="text"
                    required
