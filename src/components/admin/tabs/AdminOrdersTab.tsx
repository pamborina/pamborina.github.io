...
export const AdminOrdersTab: React.FC = () => {
  ...
  const handleAssignDriver = async (order: Order, driver: DeliveryDriver | null) => {
    if (driver && driver.status !== 'active') {
      showToast(
        'المندوب معطل بقرار الإدارة',
        `لا يمكن اختيار أو تعيين الكابتن "${driver.name}" لأنه معطل حالياً في النظام. يجب تفعيله أولاً من تبويب المناديب ليصبح متاحاً للتعيين.`,
        'error'
      );
      return;
    }
    ...
    try {
      await firebaseOrderService.assignDriverToOrder(
        order.id,
        driver ? { id: driver.id, name: driver.name, phone: driver.phone, status: driver.status } : null
      );
      showToast(
        'تم تحديث المندوب',
        driver
  ...
  const handleCleanOldOrdersSubmit = async () => {
    if (!isAdmin) {
      showToast('ليس لديك صلاحية', 'عملية تنظيف الطلبات القديمة متاحة فقط لمدير النظام الرئيسي (Admin).', 'error');
  ...
  return (
    <div className="space-y-6 text-neutral-100 font-sans" dir="rtl">
                    ...
                    <div className="flex items-center gap-2 text-neutral-400 text-[11px]">
                      {order.orderType === 'pickup' ? (
                        <>
                          ...
                          <span className="truncate">استلام من الفرع: {order.branch?.nameAr || order.branchNameAr || 'فرع بامبورينا - الطالبية هرم'}</span>
                        </>
                      ) : (
                        <>
                    ...
                    {order.orderType === 'delivery' && (
                      <div className="flex items-center justify-between p-2 rounded-xl bg-neutral-900/90 border border-neutral-700/60 text-xs">
                        <div className="flex items-center gap-1.5 min-w-0">
                          ...
                          {order.driverId && availableDrivers.find((d) => d.id === order.driverId)?.status === 'inactive' && (
                            <span className="text-[9px] px-1.5 py-0.5 rounded bg-rose-500/20 text-rose-300 font-bold border border-rose-500/40 shrink-0 flex items-center gap-0.5" title="هذا المندوب معطل حالياً بقرار الإدارة">
                              <Ban className="w-2.5 h-2.5" /> معطل
                            </span>
                          )}
                        </div>
                        <button
      ...
      {/* Order Details Modal */}
      {selectedOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/95 backdrop-blur-2xl dir-rtl animate-in fade-in duration-200">
              ...
              {selectedOrder.orderType === 'delivery' && (
                <div className="bg-neutral-950/70 p-4 rounded-2xl border border-neutral-800 space-y-2">
                  ...
                  {selectedOrder.driverId ? (
                    <>
                      {availableDrivers.find((d) => d.id === selectedOrder.driverId)?.status === 'inactive' && (
                        <div className="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-300 text-xs flex items-center gap-2">
                          <Ban className="w-4 h-4 text-rose-400 shrink-0" />
                          <span>تنبيه: الكابتن المعين لهذا الطلب ({selectedOrder.driverName}) معطل حالياً في النظام من قبل الإدارة.</span>
                        </div>
                      )}
      ...
      {assigningDriverOrder && (
        <div
          className="fixed inset-0 z-[70] flex items-center justify-center p-3 sm:p-4 bg-black/95 backdrop-blur-2xl dir-rtl animate-in fade-in duration-200 overflow-y-auto"
          onClick={(e) => {
            if (e.target === e.currentTarget && !isAssigningDriver) {
              setAssigningDriverOrder(null);
            }
          }}
        >
          <div
            className="bg-[#141416] border border-neutral-700/80 rounded-3xl p-5 sm:p-6 max-w-md w-full text-white space-y-4 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.95)] relative animate-in fade-in zoom-in-95 duration-200 my-auto"
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-lg bg-amber-500/15 text-amber-400 border border-amber-500/30">
                      #{assigningDriverOrder.orderNumber}
                    </span>
                  </div>
                  <p className="text-[11px] text-neutral-400 mt-0.5">
                    اختر المندوب المسؤول عن توصيل وتسليم هذا الطلب
                  </p>
                </div>
              </div>
              <button
                ...
                className="w-8 h-8 rounded-full bg-neutral-800/80 hover:bg-neutral-700 text-neutral-400 hover:text-white flex items-center justify-center transition-all cursor-pointer shrink-0"
                title="إغلاق"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="p-3 rounded-2xl bg-neutral-950/80 border border-neutral-800/80 flex items-center justify-between text-xs">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-7 h-7 rounded-xl bg-neutral-800 flex items-center justify-center text-amber-400 shrink-0">
                  <User className="w-3.5 h-3.5" />
                </div>
                <div className="min-w-0">
                  <span className="text-white font-bold block truncate">
                    {assigningDriverOrder.customer?.name || assigningDriverOrder.customerName || 'عميل المتجر'}
                  </span>
                  {assigningDriverOrder.customer?.address && (
              ...
              <span className="text-[11px] font-mono text-neutral-400 dir-ltr text-right shrink-0 mr-2">
                {assigningDriverOrder.customer?.phone || assigningDriverOrder.customerPhone || ''}
              </span>
            </div>

            {/* Driver status summary header */}
            <div className="flex items-center justify-between px-3.5 py-2.5 rounded-2xl bg-neutral-950/60 border border-neutral-800 text-xs">
              <span className="text-neutral-400 font-medium text-[11px]">حالة المناديب المسجلين:</span>
              <div className="flex items-center gap-2">
                <span className="text-emerald-400 font-bold text-[11px] flex items-center gap-1 bg-emerald-950/70 px-2.5 py-1 rounded-lg border border-emerald-800/50">
                  <CheckCircle2 className="w-3 h-3" /> {availableDrivers.filter((d) => d.status === 'active').length} متاح
                </span>
                {availableDrivers.filter((d) => d.status !== 'active').length > 0 && (
                  <span className="text-rose-400 font-bold text-[11px] flex items-center gap-1 bg-rose-950/70 px-2.5 py-1 rounded-lg border border-rose-800/50">
                    <Ban className="w-3 h-3" /> {availableDrivers.filter((d) => d.status !== 'active').length} معطل
                  </span>
                )}
              </div>
            </div>

            {/* Warning if all drivers are disabled */}
            {availableDrivers.length > 0 && availableDrivers.filter((d) => d.status === 'active').length === 0 && (
              <div className="p-3 rounded-2xl bg-rose-950/50 border border-rose-800/70 text-rose-300 text-xs flex items-start gap-2.5">
                <AlertCircle className="w-4 h-4 text-rose-400 shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <p className="font-bold text-white text-xs">جميع مناديب التوصيل معطلون حالياً بقرار الإدارة!</p>
                  <p className="text-[11px] text-rose-300/80 leading-relaxed">
                    لا يمكن تعيين أي مندوب لهذا الطلب لأن كل المناديب معطلون في النظام. لتفعيل كابتن، يرجى التوجه لتبويب "المناديب" وتغيير حالته إلى "نشط ومتاح".
                  </p>
                </div>
              </div>
            )}

            {/* Warning if current assigned driver was disabled by admin */}
            {assigningDriverOrder.driverId && availableDrivers.find((d) => d.id === assigningDriverOrder.driverId)?.status === 'inactive' && (
              <div className="p-3 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-center gap-2">
                ...
                <span className="text-[11px] leading-relaxed">
                  تنبيه: الكابتن المعين سابقاً لهذا الطلب ({assigningDriverOrder.driverName}) أصبح <strong>معطلاً</strong> بقرار الإدارة. يرجى اختيار كابتن نشط بديل أو إلغاء التعيين.
                </span>
              </div>
            )}

            {/* Drivers Selection List */}
            <div className="space-y-2 max-h-64 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-neutral-700">
                  ...
                  .map((driver) => {
                    ...
                    return (
                      ...
                      >
                        <button
                          type="button"
                          disabled={isAssigningDriver || !isActive}
                          onClick={() => {
                            if (!isActive) {
                              showToast(
                                'المندوب معطل بقرار الإدارة',
                                `لا يمكن اختيار الكابتن "${driver.name}" لأنه معطل حالياً في النظام. يجب تفعيله أولاً من تبويب المناديب ليصبح متاحاً للتعيين.`,
                                'error'
                              );
                              return;
                            }
                            handleAssignDriver(assigningDriverOrder, driver);
                          }}
                          className={`w-full p-3 text-right flex items-center justify-between transition-all ${
                            !isActive
                        ...
                        >
                          <div className="space-y-1 min-w-0 flex-1 ml-2">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className={`font-bold text-xs ${isActive ? 'text-white' : 'text-neutral-400'}`}>
                                {driver.name}
                              </span>

                              {isActive ? (
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-950/80 text-emerald-400 font-bold border border-emerald-800/50 flex items-center gap-1">
                                  <CheckCircle2 className="w-2.5 h-2.5" /> نشط
                                </span>
                              ) : (
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-rose-950/80 text-rose-400 font-bold border border-rose-800/50 flex items-center gap-1">
                                  <Ban className="w-2.5 h-2.5" /> معطل (غير متاح)
                                </span>
                              )}
                            ...
                            <div className="flex items-center gap-3">
                              <span className="text-[11px] font-mono text-neutral-400 dir-ltr text-right block">
                                {driver.phone}
                              </span>
                              {!isActive && (
                                <span className="text-[10px] text-rose-400/90 font-medium truncate block">
                                  معطل من الإدارة - لا يمكن تعيينه للطلبات
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="shrink-0 mr-2">
                            {isCurrent && isActive ? (
                              ...
                              <div className="flex items-center gap-1 text-neutral-500 bg-neutral-900 px-2 py-1 rounded-lg border border-neutral-800" title="معطل بقرار الإدارة">
                                <Lock className="w-3.5 h-3.5 text-neutral-500" />
                                <span className="text-[10px] font-bold text-neutral-500">مقفول</span>
                              </div>
                            ) : (
                              <div className="w-5 h-5 rounded-full border border-neutral-700 hover:border-amber-500/60 transition-colors" />
                            )}
                          </div>
                        </button>
                      </div>
            ...
            <div className="pt-3 border-t border-neutral-800 flex items-center justify-between gap-2">
              {assigningDriverOrder.driverId ? (
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-rose-950/60 hover:bg-rose-900 border border-rose-800/60 text-rose-300 text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
                >
                  <Ban className="w-3.5 h-3.5" />
                  <span>إلغاء تعيين المندوب</span>
                </button>
              ) : (
                <div className="text-[11px] text-neutral-500">
                  انقر على أي كابتن نشط للتعيين المباشر
                </div>
              )}
              <button
                type="button"
                onClick={() => setAssigningDriverOrder(null)}
                className="px-4 py-2 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-neutral-300 hover:text-white text-xs font-bold transition-all cursor-pointer mr-auto"
              >
                إغلاق
              </button>
            </div>
