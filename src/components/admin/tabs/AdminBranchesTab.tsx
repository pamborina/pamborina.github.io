...
export const AdminBranchesTab: React.FC<AdminBranchesTabProps> = ({ branches: initialBranches }) => {
  ...
  const handleToggleDeliveryStatus = async (branch: Branch) => {
    const newDeliveryStatus = !(branch.isDeliveryAvailable !== false);
    setTogglingDeliveryId(branch.id);
    try {
      await firebaseBranchService.toggleBranchDelivery(branch.id, newDeliveryStatus);
      setBranches((prev) =>
        prev.map((b) => (b.id === branch.id ? { ...b, isDeliveryAvailable: newDeliveryStatus } : b))
      );
    } catch (err: any) {
      alert(`فشل تغيير حالة توصيل الفرع: ${err?.message || err}`);
    } finally {
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {branches.map((branch) => {
          ...
          const isDeliveryAvail = branch.isDeliveryAvailable !== false;
          const isToggling = togglingId === branch.id;
          const isTogglingDelivery = togglingDeliveryId === branch.id;
              ...
              <div>
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-0.5">
                        ...
                        <span
                          className={`inline-flex items-center gap-1 text-[11px] font-bold ${
                            isDeliveryAvail ? 'text-emerald-400' : 'text-amber-400'
                          }`}
                        >
                          <Truck className="w-3.5 h-3.5" />
                          <span>{isDeliveryAvail ? 'توصيل متاح' : 'توصيل معطل'}</span>
                        </span>
                        <span className="text-neutral-500 text-xs">•</span>
                        <span className="text-xs text-neutral-400">{branch.areaAr || 'الطالبية هرم'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-end sm:items-center gap-1.5 shrink-0">
                    <button
                      type="button"
                      onClick={() => handleToggleDeliveryStatus(branch)}
                      disabled={isTogglingDelivery}
                      title="تفعيل أو تعطيل التوصيل لهذا الفرع"
                    ...
                    >
                      ...
                      <span>{isDeliveryAvail ? 'دليفري متاح' : 'دليفري معطل'}</span>
                    </button>
                ...
                <div className="space-y-2.5 text-xs text-neutral-300 bg-neutral-900/60 p-4 rounded-xl border border-neutral-700/50">
                  {!isDeliveryAvail && (
                    <div className="p-2.5 rounded-lg bg-rose-500/15 border border-rose-500/30 text-rose-300 text-[11px] flex items-center gap-2">
                      ...
                      <span>
                        {branch.deliveryDisabledReasonAr || 'خدمة التوصيل المنزلي معطلة لهذا الفرع. الطلبات المتاحة: استلام من الفرع فقط.'}
                      </span>
                    </div>
                  )}
