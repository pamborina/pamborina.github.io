...
import { MoveProductCategoryModal } from '../modals/MoveProductCategoryModal';
...
export const AdminProductsTab: React.FC<AdminProductsTabProps> = ({
  ...
  const canToggleAvailability = isAdmin || hasPermission('products.toggle_availability');
  const canDelete = isAdmin || hasPermission('products.delete');

  // Toast notification for non-blocking feedback
  const [toastMessage, setToastMessage] = useState<{ type: 'success' | 'error' | 'info'; text: string } | null>(null);

  // Category Move Modal & Quick Transfer States
  const [isMoveModalOpen, setIsMoveModalOpen] = useState(false);
  const [productsToMove, setProductsToMove] = useState<Product[]>([]);
  const [movingSingleId, setMovingSingleId] = useState<string | null>(null);
  ...
  // Quick Move Category for a single product directly from row dropdown
  const handleQuickMoveCategory = async (product: Product, newCategoryId: string) => {
    const currentCatId = product.categoryId || product.category || '';
    if (!newCategoryId || currentCatId === newCategoryId) return;

    setMovingSingleId(product.id);
    const targetCat = categories.find((c) => c.id === newCategoryId);
    const targetCatName = targetCat ? targetCat.nameAr : newCategoryId;
    const prodName = product.nameAr || product.name || product.id;

    try {
      await firebaseProductService.moveProductCategory(product.id, newCategoryId, {
        productNameAr: prodName,
        targetCategoryNameAr: targetCatName,
      });
      ...
      setToastMessage({
        type: 'success',
        text: `✅ تم نقل الصنف «${prodName}» بنجاح إلى قسم «${targetCatName}» وتحديثه فوراً!`,
      });
    } catch (err: any) {
      console.error('Quick move category error:', err);
      setToastMessage({
        type: 'error',
        text: `فشل نقل الصنف: ${err?.message || 'حدث خطأ'}`,
      });
    } finally {
      setMovingSingleId(null);
    }
  };

  // Open Move Modal
  const handleOpenMoveModal = (items: Product[]) => {
    if (!items || items.length === 0) return;
    setProductsToMove(items);
    setIsMoveModalOpen(true);
  };

  // Callback on successful category move from modal
  const handleCategoryMoveSuccess = (movedItems: Product[], targetCategory: Category) => {
    if (onProductsBulkUpdated && movedItems.length > 1) {
      onProductsBulkUpdated(movedItems);
    } else {
      movedItems.forEach((p) => onProductUpdated(p));
    }
    ...
    setToastMessage({
      ...
      text:
        movedItems.length === 1
          ? `✅ تم نقل الصنف «${movedItems[0].nameAr || movedItems[0].name}» إلى قسم «${targetCategory.nameAr}» بنجاح!`
          : `✅ تم نقل (${movedItems.length}) أصناف بنجاح إلى قسم «${targetCategory.nameAr}» وتحديثها لحظياً!`,
    });
  };
  ...
  const handleToggleSelectAllOnPage = () => {
    ...
    const allSelected = pageIds.length > 0 && pageIds.every((id) => selectedProductIds.has(id));
  ...
  return (
    <div className="space-y-6" dir="rtl">
              ...
              <table className="w-full text-right text-xs text-neutral-200">
                <thead className="bg-neutral-900/80 text-neutral-400 border-b border-neutral-700 text-[11px] uppercase">
                  <tr>
                    ...
                    <th className="py-3.5 px-4">القسم / نقل التصنيف</th>
                ...
                <tbody className="divide-y divide-neutral-700/50">
                  {paginatedProducts.map((p) => {
                    ...
                    const isMovingThis = movingSingleId === p.id;
                    const isSelected = selectedProductIds.has(p.id);
                    const currentProdCatId = p.categoryId || p.category || '';

                    return (
                      <tr
                        key={p.id}
                        className={`transition-colors ${
                          isSelected ? 'bg-amber-500/10 hover:bg-amber-500/15' : 'hover:bg-neutral-700/30'
                        }`}
                      >
                        {/* Checkbox */}
                        ...
                        <td className="py-3 px-4 w-16">
                          <div className="w-12 h-12 rounded-xl bg-neutral-900 border border-neutral-700 overflow-hidden shrink-0 flex items-center justify-center">
                            {(p.imageUrl && p.imageUrl.trim() !== '') || (p.image && p.image.trim() !== '') ? (
                              <img
                                src={(p.imageUrl && p.imageUrl.trim() !== '' ? p.imageUrl : p.image) || undefined}
                                alt={p.nameAr}
                        ...
                        {/* Category & Instant Move Selector */}
                        <td className="py-3 px-4">
                          {isMovingThis ? (
                            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs">
                            ...
                            <div className="inline-flex items-center gap-1">
                              <select
                                value={currentProdCatId}
                                onChange={(e) => handleQuickMoveCategory(p, e.target.value)}
                                className="px-2.5 py-1.5 rounded-xl bg-neutral-900 border border-neutral-700 hover:border-neutral-600 focus:border-amber-500 text-neutral-200 text-[11px] font-medium transition-colors cursor-pointer"
                                title="اختر قسم لنقل الصنف إليه فوراً بضغطة زر"
                              >
                                {categories.map((cat) => (
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg bg-neutral-900/80 border border-neutral-700 hover:border-amber-500/50 hover:bg-neutral-800 text-neutral-400 hover:text-amber-400 transition-colors"
                                title="نافذة نقل القسم المتقدمة"
                              >
                          ...
                          <button
                            ...
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                              !canToggleAvailability
                                ? 'opacity-60 cursor-not-allowed bg-neutral-800 text-neutral-500 border border-neutral-700'
                                : isAvail
                                ? 'bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/25 cursor-pointer'
                                : 'bg-rose-500/15 border border-rose-500/30 text-rose-400 hover:bg-rose-500/25 cursor-pointer'
                            }`}
                          >
                            {isToggling ? (
                        ...
                        <td className="py-3 px-4 text-left">
                          <div className="flex items-center justify-end gap-1.5">
                            {canEdit && (
                              <button
                                onClick={() => handleOpenMoveModal([p])}
                                className="p-2 rounded-xl bg-neutral-900 border border-neutral-700 text-neutral-300 hover:text-amber-400 hover:border-amber-500/50 transition-colors cursor-pointer"
                                title="نقل الصنف لقسم آخر"
                              >
                            ...
                            {canDelete && (
                              <button
                                ...
                                className="p-2 rounded-xl bg-neutral-900 border border-neutral-700 text-neutral-400 hover:text-rose-400 hover:border-rose-500/50 transition-colors cursor-pointer"
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => handleToggleSelectProduct(p.id)}
                        className="p-1.5 -mr-1 rounded hover:bg-neutral-800 text-neutral-400 hover:text-white transition-colors"
                      >
                        {isSelected ? (
                          <CheckSquare className="w-4 h-4 text-amber-400" />
                        ) : (
                          ...
                          {isMovingThis ? (
                            ...
                            <div className="flex items-center gap-1">
                              <select
                                value={currentProdCatId}
                                onChange={(e) => handleQuickMoveCategory(p, e.target.value)}
                                className="w-full px-2 py-1 rounded-lg bg-neutral-900 border border-neutral-700 text-neutral-200 text-[11px] font-medium"
                              >
                                {categories.map((cat) => (
                                  <option key={cat.id} value={cat.id}>
                                    {cat.nameAr}
                              ...
                              <button
                                type="button"
                                onClick={() => handleOpenMoveModal([p])}
                                className="p-1 rounded-lg bg-neutral-900 border border-neutral-700 text-neutral-400 hover:text-amber-400"
                                title="نقل لقسم آخر"
                              >
                                <ArrowRightLeft className="w-3.5 h-3.5" />
                              </button>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1.5">
                            {canEdit && (
                              <button
                                onClick={() => onOpenEditProduct(p)}
                                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-neutral-900 border border-neutral-700 text-neutral-300 hover:text-amber-400 text-xs font-medium cursor-pointer"
                              >
                            ...
                            {canDelete && (
                              <button
                                onClick={() => setProductToDelete(p)}
                                className="p-1.5 rounded-lg bg-neutral-900 border border-neutral-700 text-neutral-400 hover:text-rose-400 text-xs font-medium cursor-pointer"
                              >
      ...
      {/* Floating Bottom Toolbar for Bulk Selection */}
      {selectedProductIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-neutral-900/95 border border-amber-500/50 shadow-2xl shadow-black/80 rounded-2xl px-5 py-3.5 flex flex-wrap items-center justify-center gap-3 sm:gap-4 text-white backdrop-blur-md animate-slideUp">
          <div className="flex items-center gap-2 text-xs sm:text-sm font-bold">
            <span className="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse" />
            <span>
              تم تحديد <strong className="text-amber-400 text-base">{selectedProductIds.size}</strong> صنف
            </span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => {
                ...
                handleOpenMoveModal(selectedItems);
              }}
              className="bg-amber-500 hover:bg-amber-400 text-neutral-950 font-black text-xs sm:text-sm px-4 py-2 rounded-xl flex items-center gap-1.5 shadow-lg shadow-amber-500/20"
            >
              <ArrowRightLeft className="w-4 h-4" />
              <span>نقل الأصناف المحددة إلى قسم آخر...</span>
            </Button>
            ...
            <Button
              ...
              className="border-neutral-700 text-neutral-300 hover:bg-neutral-800 text-xs px-3 py-2 rounded-xl"
            >
              إلغاء التحديد
            </Button>
          </div>
      ...
      {/* Move Category Modal */}
      <MoveProductCategoryModal
        isOpen={isMoveModalOpen}
        productsToMove={productsToMove}
        categories={categories}
        allProducts={products}
        onClose={() => setIsMoveModalOpen(false)}
        onSuccess={handleCategoryMoveSuccess}
      />
          ...
          <div className="bg-neutral-900 border border-neutral-800 rounded-3xl w-full max-w-md p-6 shadow-2xl space-y-4">
            ...
            <div className="p-3.5 rounded-2xl bg-neutral-950 border border-neutral-800 flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-neutral-900 border border-neutral-700 overflow-hidden shrink-0">
                <ProductImage
                  src={productToDelete.imageUrl || productToDelete.image || null}
                  alt={productToDelete.nameAr || productToDelete.name || 'صورة المنتج'}
                  className="w-full h-full object-cover"
                />
              </div>
              <div className="flex-1 min-w-0">
