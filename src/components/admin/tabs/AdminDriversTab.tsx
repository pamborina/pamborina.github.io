...
import { DriverSettlementModal } from '../modals/DriverSettlementModal';
...
export const AdminDriversTab: React.FC = () => {
  ...
  const canSettleDrivers = isAdmin || hasPermission('drivers.settle');
  ...
  const [settlementModalDriver, setSettlementModalDriver] = useState<DeliveryDriver | null>(null);
  ...
  // 1. Subscribe to real-time drivers, orders & settlements
  useEffect(() => {
    setIsLoading(true);
    ...
    const unsubSettlements = driverSettlementService.subscribeToSettlements((settlementsList) => {
  ...
  // 2. CRITICAL: Derive Real-time Statistics & Financial Ledger from Firestore
  const driversWithStats = useMemo(() => {
    const baseWithOrderStats = deliveryDriverService.computeDriverStats(rawDrivers, orders);
    return driverSettlementService.attachFinancialStats(baseWithOrderStats, orders, settlements);
  }, [rawDrivers, orders, settlements]);
  ...
  const globalStats = useMemo(() => {
    ...
    const driverTotalSettled = driversWithStats.reduce((sum, d) => sum + (d.totalSettled || 0), 0);
    const driverTotalOutstanding = driversWithStats.reduce((sum, d) => sum + (d.outstandingBalance || 0), 0);
    const activeInProgress = driversWithStats.reduce((sum, d) => sum + (d.activeOrdersCount || 0), 0);

    return {
      total,
      active,
      totalDeliveredOrders: driverDeliveredOrders,
      totalRevenue: driverDeliveredRevenue,
      totalSettled: driverTotalSettled,
      totalOutstanding: driverTotalOutstanding,
      activeInProgress,
    };
  }, [driversWithStats]);
  ...
  return (
    <div className="space-y-6 dir-rtl text-white">
      ...
      {/* Global Analytics Cards Bar (Derived from Real Orders & Financial Ledger) */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        <div className="p-4 rounded-2xl bg-neutral-800/90 border border-neutral-700/80 shadow-md space-y-1">
          ...
          <div className="text-[10px] text-neutral-400">إجمالي رسوم التوصيل</div>
        </div>

        <div className="p-4 rounded-2xl bg-neutral-800/90 border border-teal-500/30 shadow-md space-y-1">
          <div className="flex items-center justify-between text-teal-400 text-xs">
            <span>المبالغ المسوّاة:</span>
            <Wallet className="w-4 h-4 text-teal-400" />
          </div>
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-black text-teal-300 font-mono">
              {formatPrice(globalStats.totalSettled)}
            </span>
          </div>
          <div className="text-[10px] text-teal-400/80">المسواة بالتصفية</div>
        </div>

        <div className="p-4 rounded-2xl bg-gradient-to-br from-amber-950/40 to-neutral-800/90 border border-amber-500/40 shadow-md space-y-1">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-black text-amber-400 font-mono">
              {formatPrice(globalStats.totalOutstanding)}
            </span>
          </div>
          <div className="text-[10px] text-amber-300/80">مستحقات للأسطول متبقية</div>
        </div>
      </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  {/* Real-time Order Stats & Financial Ledger Box */}
                  ...
                  <div className="mt-2.5 p-3 rounded-2xl bg-gradient-to-r from-neutral-950 to-neutral-900 border border-amber-500/30 flex items-center justify-between text-xs">
                    <div>
                      <span className="text-[10px] text-neutral-400 block">المسوّى: <strong className="text-teal-400">{formatPrice(driver.totalSettled || 0)}</strong></span>
                      <span className="text-xs font-black text-amber-400">الرصيد المستحق: {formatPrice(driver.outstandingBalance || 0)}</span>
                    </div>
                    {canSettleDrivers && (
                      <button
                        type="button"
                        onClick={() => setSettlementModalDriver(driver)}
                        className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold text-[11px] shadow-sm flex items-center gap-1.5 transition-all cursor-pointer"
                      >
                ...
                <div className="pt-3 border-t border-neutral-700/70 space-y-2">
                  <div className={`grid ${canSettleDrivers ? 'grid-cols-2' : 'grid-cols-1'} gap-2`}>
                    <button
                      type="button"
                      onClick={() => setStatementDriver(driver)}
                      className="py-2 px-2.5 rounded-xl bg-gradient-to-r from-amber-500/15 via-amber-500/25 to-amber-500/15 hover:from-amber-500/30 hover:to-amber-500/30 border border-amber-500/40 text-amber-300 font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
                    ...
                    {canSettleDrivers && (
                      <button
                        type="button"
                        onClick={() => setSettlementModalDriver(driver)}
                        className="py-2 px-2.5 rounded-xl bg-emerald-950/60 hover:bg-emerald-900/80 border border-emerald-500/40 text-emerald-300 font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-sm"
                        title="تصفية وتسوية المبالغ والعمولات المستحقة"
                      >
                        <Receipt className="w-3.5 h-3.5 text-emerald-400" />
                        <span>تسوية مالية</span>
                      </button>
                    )}
                  </div>
      ...
      <DriverStatementModal
        ...
        onOpenSettlementModal={(driver) => {
          ...
          setSettlementModalDriver(driver);
        }}
      />

      {/* Driver Financial Settlement Modal */}
      <DriverSettlementModal
        isOpen={Boolean(settlementModalDriver)}
        onClose={() => setSettlementModalDriver(null)}
        driver={settlementModalDriver}
        orders={orders}
        settlements={settlements}
        onSettlementSuccess={() => {
          showToast('تمت تسجيل عملية التسوية المالية بنجاح للفرع', 'success');
        }}
      />
