...
import { exportAndPrintReport, PrintLayoutSize } from '../../../services/pdfReportGenerator';
...
export const AdminAnalyticsTab: React.FC = () => {
  ...
  const [showPrintSizeModal, setShowPrintSizeModal] = useState<boolean>(false);

  // Time Range Filter State (Default: 'all' to ensure full visibility of all store operations)
  ...
  // Compute counts for each preset
  const presetCounts = useMemo(() => {
    const counts: Record<TimeRangePreset, number> = {
      today: 0,
      yesterday: 0,
      this_week: 0,
      last_7_days: 0,
    ...
    presets.forEach((p) => {
      counts[p] = orders.filter((o) => isOrderInDateRange(o, p)).length;
    });
    return counts;
  }, [orders]);
  ...
  // Export PDF Report handler - opens size selection modal
  const handleExportPDF = () => {
    setShowPrintSizeModal(true);
  };

  // Callback when user picks layout (A4, A5, 80mm Reset, 58mm Reset)
  ...
  return (
    <div className="space-y-8 pb-12" dir="rtl">
      {/* Printer Size & Layout Selector Modal */}
      <PrintSizeSelectorModal
        isOpen={showPrintSizeModal}
        onClose={() => setShowPrintSizeModal(false)}
        onPrint={handleConfirmPrint}
        onConfirmPrint={handleConfirmPrint}
        reportTitle={`تقرير التحليلات المالية والتشغيلية (${currentPeriodLabel})`}
        recordCount={currentFilteredOrders.length}
      />
      ...
      <div className="bg-neutral-900/90 border border-neutral-800 rounded-2xl p-4 sm:p-5 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 shadow-md">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto">
          {timeRangeButtons.map((btn) => {
            const isSelected = !isCustomRange && timeRangePreset === btn.id;
            const count = presetCounts[btn.id] || 0;
      ...
      {/* Helpful banner if selected range has 0 orders but other periods have orders */}
      {currentFilteredOrders.length === 0 && orders.length > 0 && (
        <div className="bg-gradient-to-r from-amber-500/10 via-neutral-900 to-neutral-900 border border-amber-500/40 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs shadow-md">
          <div className="flex items-center gap-2.5">
            <AlertTriangle className="w-5 h-5 text-amber-400 shrink-0" />
            <span className="text-neutral-300">
              لا توجد طلبات مسجلة في فترة <strong className="text-amber-400">({currentPeriodLabel})</strong>. يتوفر لديك <strong className="text-white font-bold">{orders.length} طلب</strong> في فترات أخرى.
            </span>
          </div>
          <button
            onClick={() => {
            ...
            className="px-3.5 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-neutral-950 font-black text-xs shrink-0 transition-all cursor-pointer shadow-md flex items-center gap-1.5"
          >
            <span>عرض جميع الفترات ({orders.length} طلب)</span>
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* 1. Products Revenue (Bamborina Store Sales - Excludes Delivery Fees) */}
        <div
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div>
              <span className="text-xs font-bold text-neutral-300 group-hover:text-emerald-300 transition-colors block">
                إيرادات مبيعات الأصناف (بامبورينا)
              </span>
              <span className="text-[10px] text-emerald-400 font-medium">
                ثمن الأصناف فقط (بدون رسوم التوصيل)
              </span>
            </div>
            ...
            <div>
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl font-black text-emerald-400 tracking-tight">
                  {kpis.productsRevenue.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
                </span>
              ...
              <div className="flex items-center gap-2 mt-1 text-[11px] text-neutral-400 font-medium">
                <span>رسوم التوصيل للدليفري: <strong className="text-amber-400 font-mono">{kpis.deliveryFeesRevenue.toLocaleString('en-US')} ج.م</strong></span>
                <span>•</span>
                <span>الإجمالي الشامل: <strong className="text-neutral-300 font-mono">{kpis.grossRevenue.toLocaleString('en-US')} ج.م</strong></span>
              </div>
            </div>
          )}
        ...
        {/* 4. Average Products Basket Value */}
        <div className="bg-neutral-800/90 border border-neutral-700/70 rounded-2xl p-5 relative overflow-hidden shadow-lg">
          <div className="flex items-center justify-between mb-3">
            <div>
              <span className="text-xs font-bold text-neutral-300 block">متوسط سلة الأصناف (AOV)</span>
              <span className="text-[10px] text-purple-400 font-medium">بدون رسوم التوصيل</span>
            </div>
            ...
            <div>
              <div className="flex items-baseline gap-1.5">
                <span className="text-3xl font-black text-white">
                  {kpis.averageProductsValue.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
                </span>
                <span className="text-xs font-bold text-neutral-400">ج.م</span>
              </div>
              <div className="text-[11px] text-neutral-400 font-medium mt-1">
                <span>شامل التوصيل: <strong className="text-neutral-300 font-mono">{kpis.averageOrderValue.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} ج.م</strong></span>
              </div>
            </div>
          )}
          <div className="mt-3 pt-3 border-t border-neutral-700/50 flex items-center justify-between text-xs text-neutral-400">
            <span>معدل قيمة المنتجات بالطلب</span>
            <span className="font-mono text-purple-400 font-bold">بامبورينا</span>
          </div>
        </div>
