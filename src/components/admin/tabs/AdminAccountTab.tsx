...
export const AdminAccountTab: React.FC<AdminAccountTabProps> = ({
  ...
  const handleCopyResult = async () => {
    ...
    await copyToClipboard(JSON.stringify(claimStatusResult, null, 2));
  ...
  if (!isAdmin) {
    return (
      <div className="p-8 bg-neutral-900 border border-rose-500/30 rounded-2xl text-center space-y-4 max-w-xl mx-auto my-12 shadow-2xl" dir="rtl">
        ...
        <h3 className="text-xl font-bold text-white">غير مصرح بالوصول (403 Access Denied)</h3>
        <p className="text-sm text-neutral-400 leading-relaxed">
          صفحة "حسابي وبياناتي" مخصصة حصرياً لمدير النظام الرئيسي (Super Admin). لا تملك تصريحاً لعرض أو تعديل بيانات الاعتماد.
        </p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-neutral-800/80 border border-neutral-700/70 rounded-2xl p-6 shadow-xl flex flex-col justify-between">
          <form onSubmit={handleChangeEmail} className="space-y-4">
            ...
            <PasswordInput
              label="كلمة المرور الحالية (لتأكيد العملية)"
              required
            ...
            <Button
              ...
              className="w-full bg-amber-500 hover:bg-amber-600 text-neutral-950 font-bold py-2.5 rounded-xl cursor-pointer"
            >
              {isUpdatingEmail ? (
                <>
