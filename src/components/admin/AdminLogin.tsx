...
export const AdminLogin: React.FC<AdminLoginProps> = ({ onLoginSuccess, onBackToStore }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Verify authorization: Super Admin OR Staff user registered in system
      if (!user.isAdmin) {
        try {
          const profile = await rbacService.fetchCurrentUserProfile();
          if (profile.isActive === false) {
            await firebaseAuthService.logoutAdmin();
            setErrorMessage('تم تجميد هذا الحساب من قِبل إدارة النظام. يرجى التواصل مع المشرف.');
            return;
          }
        } catch (rbacErr: any) {
          await firebaseAuthService.logoutAdmin();
          const errText = rbacErr?.message || '';
          if (errText.includes('USER_DEACTIVATED')) {
            setErrorMessage('تم تجميد هذا الحساب من قِبل إدارة النظام.');
          } else {
            setErrorMessage('هذا الحساب غير مسجل ضمن طاقم العمل أو مستخدمي لوحة التحكم.');
          }
          return;
        }
      }

      // 3. Authorized Admin or Staff User
      onLoginSuccess(user);
    } catch (err: any) {
