...
export const AdminBranchModal: React.FC<AdminBranchModalProps> = ({
  ...
  const [deliveryDisabledReasonAr, setDeliveryDisabledReasonAr] = useState('');
  ...
  useEffect(() => {
    if (branch) {
      ...
      setIsDeliveryAvailable(branch.isDeliveryAvailable !== false);
      setDeliveryDisabledReasonAr(branch.deliveryDisabledReasonAr || '');
      ...
      const initialMapUrl =
        currentMap && !currentMap.includes('30.0035,31.1965') && !currentMap.includes('CUz4tnN9Gi6c1awU9')
          ? currentMap
          : 'https://maps.app.goo.gl/kBFkLDC3xrRVxz2R6';
      ...
      setDeliveryDisabledReasonAr('');
      setDeliveryFee(0);
      setMinOrderAmount(50);
      setDeliveryEstimateMinutes(30);
      setMapUrl('https://maps.app.goo.gl/kBFkLDC3xrRVxz2R6');
    }
    setError(null);
  }, [branch, isOpen]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const branchPayload: Omit<Branch, 'id'> = {
        ...
        cityAr: 'الطالبية هرم',
        areaAr: areaAr.trim() || 'الطالبية هرم',
        phone: phone.trim(),
        ...
        deliveryDisabledReasonAr: deliveryDisabledReasonAr.trim() || undefined,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-sm overflow-y-auto" dir="rtl">
      <div className="bg-neutral-900 border border-neutral-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl my-auto max-h-[90vh] max-h-[90dvh] flex flex-col">
        ...
        {/* Form */}
        <form id="branch-modal-form" onSubmit={handleSave} className="p-4 sm:p-6 space-y-4 overflow-y-auto flex-1 custom-scrollbar">
          ...
          <div>
            ...
            <input
              ...
              placeholder="مثال: 97 شارع عثمان محرم، الطالبية، هرم"
              required
              className="w-full px-3.5 py-2.5 rounded-xl bg-neutral-950 border border-neutral-800 text-xs text-white focus:outline-none focus:border-amber-500"
            />
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-neutral-950 border border-neutral-800 flex items-center justify-between">
            <span className="text-xs font-semibold text-white">حالة الفرع (استقبال الطلبات)</span>
          ...
          {/* Delivery Availability Control */}
          <div className="p-3.5 rounded-xl bg-neutral-950 border border-neutral-800 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-xs font-semibold text-white block">خدمة التوصيل المنزلي للفرع (دليفري)</span>
                <span className="text-[11px] text-neutral-400">عند التعطيل، يمنع اختيار التوصيل لهذا الفرع تلقائياً</span>
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 accent-amber-500"
                />
                <span className={`text-xs font-bold ${isDeliveryAvailable ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {isDeliveryAvailable ? 'التوصيل متاح' : 'التوصيل معطل'}
                </span>
              </label>
            </div>
            {!isDeliveryAvailable && (
              <div>
                <label className="block text-[11px] text-neutral-400 mb-1">سبب تعطيل التوصيل للفرع (يظهر للعميل):</label>
                <input
                  type="text"
                  value={deliveryDisabledReasonAr}
                  onChange={(e) => setDeliveryDisabledReasonAr(e.target.value)}
                  placeholder="خدمة التوصيل معطلة مؤقتاً لهذا الفرع بسبب ضغط العمل أو أعمال الصيانة"
                  className="w-full px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-800 text-xs text-white placeholder:text-neutral-600 focus:outline-none focus:border-amber-500"
                />
              </div>
            )}
          </div>
        ...
        {/* Sticky Footer */}
        <div className="px-5 sm:px-6 py-4 border-t border-neutral-800 bg-neutral-900/95 flex items-center justify-between gap-3 shrink-0">
          ...
          <Button
            ...
            disabled={loading}
            className="bg-amber-500 hover:bg-amber-600 text-neutral-950 font-bold px-6 py-2.5 rounded-xl cursor-pointer"
          >
            {loading ? (
              <>
