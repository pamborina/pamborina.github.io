...
const HERO_OFFERS_CACHE_KEY = 'pamborina_hero_offers_cache_v4';
const HERO_OFFERS_CONFIG_CACHE_KEY = 'pamborina_hero_offers_config_cache_v4';
...
/**
 * Sanitizes offer text to ensure strict adherence to فرع الطالبية هرم
 */
export function sanitizeHeroOffer(offer: HeroOffer): HeroOffer {
  const sanitized = { ...offer };
  if (sanitized.badgeAr && sanitized.badgeAr.includes('الجيزة')) {
    sanitized.badgeAr = sanitized.badgeAr.replace(/في الجيزة|الجيزة/g, 'في الطالبية هرم');
  }
  if (sanitized.titleAr && sanitized.titleAr.includes('الجيزة')) {
    sanitized.titleAr = sanitized.titleAr.replace(/في الجيزة|الجيزة/g, 'في الطالبية هرم');
  }
  if (sanitized.subtitleAr && sanitized.subtitleAr.includes('الجيزة')) {
    sanitized.subtitleAr = sanitized.subtitleAr.replace(/في الجيزة|الجيزة/g, 'في الطالبية هرم');
  }
  return sanitized;
}
...
export const DEFAULT_HERO_OFFERS: HeroOffer[] = [
  ...
  },
  {
    ...
    badgeAr: '🔥 الأكثر طلباً في الطالبية هرم',
  ...
  },
  {
    ...
    subtitleAr: 'وجبات حادقة سخنة ومقرمشة محضرة طازجة من فرع بامبورينا - الطالبية هرم.',
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map(sanitizeHeroOffer);
      }
    }
  } catch (e) {
    console.warn('Could not read cached hero offers:', e);
  }
  return DEFAULT_HERO_OFFERS.map(sanitizeHeroOffer);
}

function saveOffersToLocalStorage(offers: HeroOffer[]): void {
  try {
    const sanitized = offers.map(sanitizeHeroOffer);
    memoryOffers = [...sanitized];
    localStorage.setItem(HERO_OFFERS_CACHE_KEY, JSON.stringify(sanitized));
    if (typeof window !== 'undefined') {
      window.dispatchEvent(
        new CustomEvent(HERO_OFFERS_CHANGED_EVENT, { detail: sanitized })
      );
    }
  } catch (e) {
    ...
    try {
      const unsubFirestore = onSnapshot(
        ...
        (snap) => {
          ...
          snap.forEach((d) => {
            if (d.exists()) {
              const rawData = d.data() as HeroOffer;
              const sanitized = sanitizeHeroOffer({
                ...rawData,
                id: d.id,
              });
              list.push(sanitized);

              // If the raw doc contained الجيزة, quietly sanitize it in Firestore
              if (
                db &&
                ((rawData.badgeAr && rawData.badgeAr.includes('الجيزة')) ||
                  (rawData.titleAr && rawData.titleAr.includes('الجيزة')) ||
                  (rawData.subtitleAr && rawData.subtitleAr.includes('الجيزة')))
              ) {
                setDoc(doc(db, HERO_OFFERS_COLLECTION, d.id), sanitized, { merge: true }).catch(() => {});
              }
            }
          });
  ...
  async getHeroOffers(): Promise<HeroOffer[]> {
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        snap.forEach((d) => {
          if (d.exists()) {
            list.push(sanitizeHeroOffer({ ...(d.data() as HeroOffer), id: d.id }));
          }
        });
        list.sort((a, b) => (a.sortOrder ?? 1) - (b.sortOrder ?? 1));
        saveOffersToLocalStorage(list);
