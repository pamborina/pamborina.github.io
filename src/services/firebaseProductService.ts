...
export const firebaseProductService = {
  ...
  /**
   * Fast one-click category transfer for a single product with live Firestore and audit log sync.
   */
  async moveProductCategory(
    productId: string,
    targetCategoryId: string,
    options?: { productNameAr?: string; targetCategoryNameAr?: string }
  ): Promise<void> {
    if (!productId || !targetCategoryId) {
      throw new Error('بيانات غير مكتملة لنقل القسم');
    }
    ...
    if (isFirebaseConfigured() && db) {
      try {
        ...
        console.log(`✅ [Firestore] Product [${productId}] moved to category [${targetCategoryId}]`);
      } catch (err: any) {
        console.warn(`⚠️ [Firestore] moveProductCategory warning:`, err);
      }
    }
    ...
    try {
      await auditLogService.logAdminAction({
        ...
        targetType: 'product',
        targetId: productId,
        summaryAr: `تم نقل الصنف (${options?.productNameAr || productId}) إلى قسم (${options?.targetCategoryNameAr || targetCategoryId})`,
        metadata: { targetCategoryId },
      });
    } catch {}
  },

  /**
   * Bulk transfer multiple products to a new category in a single atomic batch.
   */
  async bulkMoveProductsCategory(
    productIds: string[],
    targetCategoryId: string,
    targetCategoryNameAr?: string
  ): Promise<void> {
    if (!productIds || productIds.length === 0 || !targetCategoryId) return;
    ...
    if (isFirebaseConfigured() && db) {
      try {
        ...
        await batch.commit();
        console.log(`✅ [Firestore] Bulk moved ${productIds.length} products to category [${targetCategoryId}]`);
      } catch (err: any) {
        console.error('❌ [Firestore] Failed to bulk move products category:', err);
        throw new Error('فشل نقل الأصناف إلى القسم الجديد في قاعدة البيانات');
      }
    }

    try {
      await auditLogService.logAdminAction({
        action: 'bulk_move_product_category',
        targetType: 'product',
        targetId: 'bulk',
        summaryAr: `تم نقل (${productIds.length}) أصناف دفعة واحدة إلى قسم (${targetCategoryNameAr || targetCategoryId})`,
        metadata: { count: productIds.length, targetCategoryId },
      });
    } catch {}
  },
};
