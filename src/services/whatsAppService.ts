...
class WhatsAppService {
  ...
  public formatOrderMessage(payload: GuestOrderPayload): string {
    ...
    const deliveryFeeLabel = payload.orderType === 'pickup'
      ...
      : (payload.deliveryFee > 0 ? `${payload.deliveryFee} ج.م` : 'يتم تحديدها بناءً على المسافة وعدد الكيلومترات من قِبل الإدارة فوراً');
    ...
    const totalSection = payload.orderType === 'pickup' || payload.deliveryFee > 0
      ? `▫️ *المبلغ الإجمالي:* *${payload.grandTotal} ج.م*`
      : `▫️ *المبلغ الإجمالي:* *${payload.subtotal} ج.م* (+ رسوم التوصيل تحدد فوراً بناءً على المسافة)`;
