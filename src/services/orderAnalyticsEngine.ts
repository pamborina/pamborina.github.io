...
export interface DailyRevenuePoint {
  ...
  revenue: number; // products revenue on this day
  productsRevenue?: number;
...
export function getOrderProductsTotal(order: Order): number {
  if (!order) return 0;

  // 1. Check explicit subtotal in order or pricing snapshot
  const subtotal = order.pricing?.subtotal ?? order.subtotal;
  if (subtotal !== undefined && subtotal !== null) {
    const num = Number(subtotal);
    if (!isNaN(num) && num >= 0) return Math.round(num * 100) / 100;
  }

  // 2. Calculate sum of items if items array exists
  if (Array.isArray(order.items) && order.items.length > 0) {
    const itemsSum = order.items.reduce((sum, rawItem: any) => {
      const qty = Number(rawItem?.quantity) || 1;
      const price = Number(rawItem?.totalPrice) || (Number(rawItem?.unitPrice) || Number(rawItem?.product?.price) || 0) * qty;
      return sum + (isNaN(price) || price < 0 ? 0 : price);
    }, 0);
    if (itemsSum > 0) {
  ...
  // 3. Fallback: Total minus delivery fee
...
export function getOrderDeliveryFee(order: Order): number {
  if (!order) return 0;
  const fee = order.deliveryFee ?? order.pricing?.deliveryFee ?? 0;
...
export function calculateOrderKPIs(orders: Order[]): CoreKPIs {
  if (!Array.isArray(orders) || orders.length === 0) {
    return {
      ...
      grossRevenue: 0,
      productsRevenue: 0,
      deliveryFeesRevenue: 0,
      averageOrderValue: 0,
      averageProductsValue: 0,
      deliveryOrders: 0,
      pickupOrders: 0,
      unknownTypeOrders: 0,
  ...
  for (const order of orders) {
    ...
    if (status === 'completed' || status === 'delivered') {
      ...
      const delivFee = getOrderDeliveryFee(order);
      const prodRev = getOrderProductsTotal(order);
      grossRevenue += orderTot;
      deliveryFeesRevenue += delivFee;
      productsRevenue += prodRev;
    } else if (status === 'cancelled') {
      cancelledOrders++;
    } else if (['pending', 'confirmed', 'preparing', 'ready', 'out_for_delivery'].includes(status)) {
      activeOrders++;
    }
  ...
  deliveryFeesRevenue = Math.round(deliveryFeesRevenue * 100) / 100;

  const averageOrderValue = completedOrders > 0 
    ? Math.round((grossRevenue / completedOrders) * 100) / 100 
    : 0;

  const averageProductsValue = completedOrders > 0 
    ? Math.round((productsRevenue / completedOrders) * 100) / 100 
    : 0;
...
export function calculateDailyRevenue(orders: Order[]): DailyRevenuePoint[] {
  ...
  const dayMap: Record<string, { orders: number; revenue: number; productsRevenue: number; deliveryRevenue: number }> = {};

  for (const order of orders) {
    if (!order || (!['completed', 'delivered'].includes((order.status || '').toLowerCase())) || !order.createdAt) continue;
    const day = toCairoDateString(order.createdAt);
    if (!day) continue;

    if (!dayMap[day]) {
      dayMap[day] = { orders: 0, revenue: 0, productsRevenue: 0, deliveryRevenue: 0 };
    }
    ...
    dayMap[day].orders += 1;
    dayMap[day].revenue += orderTot;
    dayMap[day].productsRevenue += prodRev;
    dayMap[day].deliveryRevenue += delivFee;
  }
  ...
  return sortedDays.map((day) => ({
    ...
    productsRevenue: Math.round(dayMap[day].productsRevenue * 100) / 100,
    deliveryRevenue: Math.round(dayMap[day].deliveryRevenue * 100) / 100,
  }));
}
...
export function calculateBranchPerformance(orders: Order[]): BranchPerformanceMetric[] {
  ...
  for (const order of orders) {
    ...
    if (status === 'completed' || status === 'delivered') {
      ...
      const prod = getOrderProductsTotal(order);
      const fee = getOrderDeliveryFee(order);
      b.revenue += tot;
      b.productsRevenue = (b.productsRevenue || 0) + prod;
      b.deliveryRevenue = (b.deliveryRevenue || 0) + fee;
    } else if (status === 'cancelled') {
      b.cancelledOrders += 1;
    } else if (['pending', 'confirmed', 'preparing', 'ready', 'out_for_delivery'].includes(status)) {
      b.activeOrders += 1;
  ...
  return Object.values(branchMap)
    .map((b) => {
      ...
      const prodRev = Math.round((b.productsRevenue || 0) * 100) / 100;
      const delivRev = Math.round((b.deliveryRevenue || 0) * 100) / 100;
...
export function calculateProductPerformance(orders: Order[]): ProductPerformanceMetric[] {
  ...
  for (const order of orders) {
    if (!order || (!['completed', 'delivered'].includes((order.status || '').toLowerCase())) || !Array.isArray(order.items)) continue;
...
export function calculatePaymentMethodPerformance(orders: Order[]): PaymentMethodMetric[] {
  ...
  for (const order of orders) {
    ...
    if (status === 'completed' || status === 'delivered') {
      ...
      const fee = getOrderDeliveryFee(order);
      p.revenue += tot;
      p.productsRevenue += prod;
      p.deliveryRevenue += fee;
      totalCompletedRevenue += tot;
    }
  }
  ...
  return Object.values(payMap)
    .map((p) => {
      const rev = Math.round(p.revenue * 100) / 100;
      const prodRev = Math.round((p.productsRevenue || 0) * 100) / 100;
      const delivRev = Math.round((p.deliveryRevenue || 0) * 100) / 100;
      const pct = totalCompletedRevenue > 0 ? Math.round((rev / totalCompletedRevenue) * 10000) / 100 : 0;
      return {
...
export function calculateOrderTypePerformance(orders: Order[]): OrderTypeMetric[] {
  ...
  const typeMap: Record<string, {
    ...
    delivery: { orderType: 'delivery', orderTypeAr: 'توصيل للمنزل 🛵', totalOrders: 0, completedOrders: 0, revenue: 0, productsRevenue: 0, deliveryRevenue: 0 },
    pickup: { orderType: 'pickup', orderTypeAr: 'استلام من الفرع 🏪', totalOrders: 0, completedOrders: 0, revenue: 0, productsRevenue: 0, deliveryRevenue: 0 },
    unknown: { orderType: 'unknown', orderTypeAr: 'غير محدد ❓', totalOrders: 0, completedOrders: 0, revenue: 0, productsRevenue: 0, deliveryRevenue: 0 },
  };
  ...
  for (const order of orders) {
    ...
    if (status === 'completed' || status === 'delivered') {
      ...
      const fee = getOrderDeliveryFee(order);
      t.revenue += tot;
      t.productsRevenue += prod;
      t.deliveryRevenue += fee;
      totalCompletedRevenue += tot;
    }
  }
  ...
  return Object.values(typeMap)
    ...
    .map((t) => {
      ...
      const prodRev = Math.round((t.productsRevenue || 0) * 100) / 100;
      const delivRev = Math.round((t.deliveryRevenue || 0) * 100) / 100;
      const pct = totalCompletedRevenue > 0 ? Math.round((rev / totalCompletedRevenue) * 10000) / 100 : 0;
      return {
