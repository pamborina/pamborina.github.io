...
import { Order, AuditLog, DeliveryDriver } from '../types';
import { 
...
export function generateSingleInvoiceHtml(order: Order, options?: { autoPrint?: boolean; initialLayout?: PrintLayoutSize }): string {
  ...
  const branchName = order.branch?.nameAr || order.branchNameAr || 'فرع الطالبية';
  ...
  const content = `
    ...
    <div class="brand-header">
      ...
      <div class="brand-meta">📍 ${branchName} • هاتف: 01012345678</div>
...
export function generateDriverReportHtml(
  ...
  const completedOrders = driverOrders.filter((o) => (o.status || '').toLowerCase() === 'completed');
  ...
  const content = `
    ...
    <div class="brand-header">
      <img src="${Images.logo}" alt="شعار بامبورينا" class="brand-logo-img" crossorigin="anonymous" referrerpolicy="no-referrer" />
      <div class="brand-title">حلواني بامبورينا</div>
      <div class="doc-title">كشف حساب كابتن التوصيل</div>
    ...
    <!-- Summary Box -->
    <div class="kpi-box">
      <div class="kpi-title">إجمالي رسوم التوصيل المستحقة</div>
    ...
    <div class="kv-row">
      <span class="kv-label">إجمالي قيمة مبيعات الطلبات:</span>
    ...
    <!-- Settlement Box -->
    <div class="grand-total">
      <span>صافي مستحقات الكابتن:</span>
      <span class="font-mono">${totalDeliveryFees.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })} ج.م</span>
    </div>
    ...
    <!-- Footer -->
    <div class="receipt-footer">
      <div>كشف حساب معتمد لكابتن التوصيل</div>
      <div>حلواني بامبورينا • قسم الحركة واللوجستيات</div>
    </div>
  `;

  return getThermalShell(`كشف حساب كابتن ${driver.name}`, autoPrint, content);
}
...
export function exportAndPrintDriverReport(
  ...
  const htmlContent = generateDriverReportHtml(driver, driverOrders, { ...options, autoPrint: mode === 'print' });
  openThermalPrintWindow(htmlContent);
}
