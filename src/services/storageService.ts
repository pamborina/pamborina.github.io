...
class StorageService {
  ...
  public getGuestOrders<T = any[]>(defaultValue: T = [] as unknown as T): T {
    ...
    if (!Array.isArray(orders)) return defaultValue;
    ...
    for (const o of orders) {
      if (!o) continue;
      if (resetTime && o.createdAt) {
        const createdTime = new Date(o.createdAt).getTime();
        if (!isNaN(createdTime) && createdTime <= resetTime) continue;
      }

      const cleanId = (o.id || '').toString().trim();
      const cleanNum = (o.orderNumber || '').toString().trim().toUpperCase();
    ...
    // Auto-clean duplicates from localStorage if found
    if (deduplicated.length !== orders.length) {
      this.setItem(STORAGE_KEYS.ORDERS, deduplicated);
    }
