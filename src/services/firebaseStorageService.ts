...
export const firebaseStorageService = {
  ...
  /**
   * Uploads a category thumbnail image file.
   * Compresses image to high-efficiency square thumbnail format,
   * uploads to Storage if available, and guarantees instant fallback Data URL.
   */
  async uploadCategoryImage(file: File, categoryId: string): Promise<string> {
    let compressedFallbackUrl = '';
    try {
      compressedFallbackUrl = await compressImageToDataUrl(file, 500, 500, 0.85);
    } catch {
      // ignore
    }
    ...
    if (isFirebaseConfigured() && storage) {
      try {
        const fileExtension = file.name.split('.').pop() || 'webp';
        const storageRef = ref(storage, `categories/${categoryId}_${Date.now()}.${fileExtension}`);

        const uploadPromise = (async () => {
          const snapshot = await uploadBytes(storageRef, file, {
            contentType: file.type || 'image/jpeg',
            customMetadata: { categoryId, type: 'category_thumbnail' },
          });
          return await getDownloadURL(snapshot.ref);
        })();
        ...
        console.warn('⚠️ [Firebase Storage] Category upload fallback to compressed Data URL:', err?.message || err);
      }
    }
  ...
  /**
   * Uploads a review screenshot image file with high-definition compression.
   */
  async uploadReviewScreenshot(file: File, reviewId: string): Promise<string> {
    let compressedFallbackUrl = '';
    try {
      compressedFallbackUrl = await compressImageToDataUrl(file, 1200, 1200, 0.85);
    } catch {
      // ignore
    }
    ...
    if (isFirebaseConfigured() && storage) {
      try {
        const fileExtension = file.name.split('.').pop() || 'webp';
        const storageRef = ref(storage, `reviews/${reviewId}_${Date.now()}.${fileExtension}`);

        const uploadPromise = (async () => {
          const snapshot = await uploadBytes(storageRef, file, {
            contentType: file.type || 'image/jpeg',
            customMetadata: { reviewId, type: 'review_screenshot' },
          });
          return await getDownloadURL(snapshot.ref);
        })();
        ...
        console.warn('⚠️ [Firebase Storage] Review screenshot upload fallback:', err?.message || err);
      }
    }
  ...
  /**
   * Deletes a review screenshot or product image from Storage using its full URL or path ref.
   */
  ...
  async deleteReviewScreenshot(imageUrl: string): Promise<void> {
    return this.deleteProductImage(imageUrl);
  },
};
