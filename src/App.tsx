...
import { syncTimeWithServer } from './utils/dateFormatter';
...
import { FloatingWhatsAppWidget } from './components/navigation/FloatingWhatsAppWidget';
...
function MainAppContent() {
  ...
  // Auto-track order from URL query parameter (e.g., ?track=ORDER_NUM or ?order=ORDER_NUM)
  useEffect(() => {
    try {
      const params = new URLSearchParams(window.location.search);
      const trackVal = params.get('track') || params.get('order');
      if (trackVal) {
        const cleanTrackVal = trackVal.trim();
        if (cleanTrackVal) {
          setTrackingOrderNumber(cleanTrackVal);
          setIsOrderTrackingOpen(true);
          
          // Clear query params from browser address bar smoothly without page reload
          const cleanUrl = window.location.origin + window.location.pathname + window.location.hash;
          window.history.replaceState({}, document.title, cleanUrl);
        }
      }
    } catch (e) {
      console.warn('⚠️ Error reading URL tracking params:', e);
    }
  }, []);

  // Synchronize system clock with the server on mount to prevent any device time alignment issues
  useEffect(() => {
    syncTimeWithServer();
    const interval = setInterval(syncTimeWithServer, 5 * 60 * 1000);
    return () => clearInterval(interval);
  }, []);

  const handleOpenOrderTracking = (orderNumber?: string) => {
    const targetNum = orderNumber || localStorage.getItem('bamborina_last_order_number') || '';
    setTrackingOrderNumber(targetNum);
    setIsOrderTrackingOpen(true);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0F0B08] text-[#F7F3E8] font-sans antialiased dir-rtl flex flex-col selection:bg-[#D4AF37] selection:text-black">
          ...
          <div className="space-y-6">
            <div className="border-b border-[#2C1F16] pb-4">
              ...
              <p className="text-xs text-[#C8BFB0] mt-1">
                خدمة التوصيل السريع من فرع بامبورينا - الطالبية هرم
              </p>
            </div>
      ...
      {/* Floating Customer Support & WhatsApp Hub Widget */}
