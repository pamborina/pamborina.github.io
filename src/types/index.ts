...
export interface Order {
  ...
  // Manual Delivery Fee Allocation
  deliveryFeeManuallySet?: boolean;
  deliveryFeeNotes?: string;
  deliveryFeeUpdatedAt?: string;
  deliveryFeeUpdatedBy?: string;
}
...
export interface DriverSettlement {
  ...
  amount: number; // Settlement amount in EGP
  amountCents?: number; // Amount in piasters/قروش for exact integer math
  paymentMethod: 'cash' | 'bank_transfer' | 'e_wallet' | 'other';
  notes?: string;
  createdAt: string; // ISO string or Cairo formatted
  createdAtServer?: any;
  createdBy: string; // Admin UID
  createdByEmail: string; // Admin Email
  grossRevenueAtSettlement?: number;
  previousSettledAtSettlement?: number;
  outstandingAtSettlement?: number;
}

export interface DriverFinancialSummary {
  driverId: string;
  driverName: string;
  grossRevenue: number; // SUM(deliveryFee of completed orders)
  totalSettled: number; // SUM(settlement.amount)
  outstandingBalance: number; // grossRevenue - totalSettled
...
export interface DeliveryDriver {
  ...
  createdAt: string;
  updatedAt: string;
  // Real-time derived analytics from actual completed orders & financial ledger
  orderCount?: number;
  deliveredRevenue?: number; // Gross delivery fees
  totalSettled?: number; // Total settled amount
  outstandingBalance?: number; // Outstanding balance (grossRevenue - totalSettled)
  activeOrdersCount?: number;
...
export type ContactChannelCategory = 'general' | 'support' | 'complaints' | 'custom';
...
export interface SiteSettings {
  ...
  // Contact & Support WhatsApp Multi-Numbers Channels
  ...
  // Paper Menu Banner Settings (خانة صور المنيو المطبوع الأصلي بالصفحة الرئيسية)
  ...
  // Free Delivery Bar Settings (خانة وشريط التوصيل المجاني)
